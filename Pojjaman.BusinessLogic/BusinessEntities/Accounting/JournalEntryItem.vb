...
#Region "Class Methods"
        Public Sub Populate(ByVal dt As TreeTable)
            ...
            Next
        End Sub
        Public Function GetMappingItem(ByVal mapping As String) As JournalEntryItem
            ...
            Next
        End Function
        Public Function GetMappingItems(ByVal mapping As String) As JournalEntryItemCollection
			...
			Return ret
        End Function
		Public Function GetMappingItems(ByVal mapping As String, ByVal glfi As GLFormatItem) As JournalEntryItemCollection
			Dim ret As New JournalEntryItemCollection
			For Each item As JournalEntryItem In Me
				If item.Mapping.ToLower = mapping.ToLower Then
					ret.Add(item)
		...
		Public Function GetMappingItems(ByVal glfi As GLFormatItem) As JournalEntryItemCollection
			Dim ret As New JournalEntryItemCollection
			Dim acct As Account
			If glfi.Mapping.ToLower.EndsWith("d") Or glfi.Mapping.ToLower.EndsWith("w") Then
				For Each item As JournalEntryItem In Me
					If item.Mapping.ToLower = glfi.Mapping.ToLower Then
						Select Case glfi.FieldType.Value
							Case 1							 'GeneralAccount
								...
								End If
							Case 2, 3							 'mix
								If TypeOf glfi.Field Is GeneralAccount AndAlso (item.Account Is Nothing OrElse Not item.Account.Originated) Then
									acct = CType(glfi.Field, GeneralAccount).Account
									acct = New Account(acct.Code)
								Else
									acct = item.Account
					...
					End If
				Next
				Return ret				'Return GetMappingItems(glfi.Mapping)
			End If
