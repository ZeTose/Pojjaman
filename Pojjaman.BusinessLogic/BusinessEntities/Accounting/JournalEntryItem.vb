...
#Region "Constructors"
    ...
    Protected Sub Construct(ByVal dr As DataRow, ByVal aliasPrefix As String)
      With Me
        ...
        End If
        If dr.Table.Columns.Contains(aliasPrefix & "gli_mapping") AndAlso Not dr.IsNull(aliasPrefix & "gli_mapping") Then
          .m_mapping = CStr(dr(aliasPrefix & "gli_mapping"))
        End If
      End With
    End Sub
...
#Region "Properties"
    ...
    Public Property CustomRefstr As String
    Public Property CustomRefType As String    Public Property AtomNote As String
...
#Region "Class Methods"
    ...
    Private Shared Function GetAccount(ByVal glfi As GLFormatItem, ByVal ji As JournalEntryItem) As Account
      ...
      Select Case glfi.FieldType.Value
        Case 1                'GeneralAccount
          ...
          Else
            acct = glfi.Account
          End If
        Case 2, 3               'mix และ Dynamics รับมาจาก Entity
      ...
      Return acct
    End Function
    'Collection ชุดที่ได้มาจาก Class หลัก
    ...
    'Collection ชุดที่ได้มาจาก Class หลัก
    'เปลี่ยนจากเดิม ใช้กับ INewGLAble จะเอาชุด JI ที่เป็น Detail ส่งมาเลย แล้วมากรุ๊ปอีกที
    Public Function NewGetMappingItems(ByVal glfi As GLFormatItem) As JournalEntryItemCollection
      Dim ret As New JournalEntryItemCollection
      'แบบ แยกเลย D และ W โดยไม่ต้องสนใจว่า entity มี Maping ที่เป็น D หรือ W หรือเปล่า
      'ยังมีปัญหาว่า ถ้ามีทั้ง D และ W จะส่งอะไรมา 
      'ต้องส่ง W มาเพราะละเอียดกว่า แล้วทางนี้มาจัดเป็น D เอง
      'เพราะ ใน GLAtom ต้องเป็น W
      Dim map As String

      If glfi.Mapping.ToLower.EndsWith("d") OrElse glfi.Mapping.ToLower.EndsWith("w") Then
        map = Mid(glfi.Mapping, glfi.Mapping.Length - 1, 1).ToLower
      Else
        map = glfi.Mapping.ToLower
      End If
      For Each item As JournalEntryItem In Me
        If item.Mapping.ToLower = map Then
          item.Account = GetAccount(glfi, item)
          Dim cc As CostCenter = glfi.CostCenter
          If Not cc Is Nothing AndAlso cc.Originated Then
            item.CostCenter = cc
      ...
      '''ไม่ไปข้าง ล่าง ไม่ต้อง Group อะไรทั้งสิ้น
      'ไม่เข้าใจว่าทำไมต้องใช้ regular expression
      Dim reg As New Regex("[A-Za-z]+[0-9]*(\.?[0-9]+)+")
      Dim dt As New TreeTable
      dt.Columns.Add(New DataColumn("Mapping", GetType(String)))
      dt.Columns.Add(New DataColumn("CostCenter", GetType(String)))
      ...
      For Each ccItem As TreeRow In ccTable.Childs 'As DictionaryEntry In ccHash
        ...
        For Each acctItem As TreeRow In acctTable.Childs 'DictionaryEntry In acctHash
          ...
          Dim itemnote As String = ""
          For Each row As DataRow In rows
            Dim maps As String = row("Mapping").ToString
            Dim value As Decimal = CDec(row("Value"))
            itemnote = row("Note").ToString
            If mapValHash.Contains(maps.ToLower) Then
              mapValHash(maps.ToLower) = CDec(mapValHash(maps.ToLower)) + value
            Else
              mapValHash(maps.ToLower) = value
            End If
          Next
          Dim mapping As String = glfi.Mapping.ToLower
          For Each m As Match In reg.Matches(glfi.Mapping.ToLower)
