...
#Region "Methods"
    ...
    '    Return Nothing
    'End Function
    Private Function GetIsChildActive() As Boolean
      Dim ds As DataSet = SqlHelper.ExecuteDataset(SimpleBusinessEntityBase.ConnectionString, CommandType.StoredProcedure, "GetIsChildActive", New SqlParameter("@cc_id", Me.Id))

      Dim isChildActive As Boolean = False
      For Each row As DataRow In ds.Tables(0).Rows
        isChildActive = CBool(row("childActive"))
      Next
      ...
      With Me
        ...
        If Me.Originated Then
          If Not Me.IsActive Then
            'ถ้าลูก ๆ ยังคง Active อยู่แม่ก็ห้าม UnActive
            If Me.GetIsChildActive Then
              Return New SaveErrorException(Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.CostCenter.NotSaveUntilChildCCActive}"), vbCrLf)
            End If
          End If
        End If
...
#Region "Shared"
    ...
    Public Shared Function GetCostCenter(ByVal id As Integer, ByVal viewType As ViewType) As CostCenter
      ...
      Return cc
    End Function
    Public Shared Sub SetMinimumCC(ByVal cc As CostCenter, ByVal dr As DataRow)
      ...
      cc.RootWBSId = drh.GetValue(Of Integer)("wbs_id")
      cc.Type = New CostCenterType(drh.GetValue(Of Integer)("cc_type"))
