...
#Region "Properties"
        ...
        Public Overrides ReadOnly Property DetailPanelTitle() As String
            Get
                Return "${res:Longkong.Pojjaman.BusinessLogic.POTerm.DetailLabel}"
            End Get
        End Property
        Public Overrides ReadOnly Property DetailPanelIcon() As String
            Get
                Return "Icons.16x16.PLESystem"
            End Get
        End Property
        ...
        Public Overrides ReadOnly Property ListPanelTitle() As String
            Get
                Return "${res:Longkong.Pojjaman.BusinessLogic.POTerm.ListLabel}"
            End Get
        End Property
...
#Region "Shared"
        Public Shared Function GetPOTerm(ByVal txtCode As TextBox, ByVal txtName As TextBox, ByRef oldPOTerm As POTerm) As Boolean
            Dim myPOTerm As New POTerm(txtCode.Text)
            If txtCode.Text.Length <> 0 AndAlso Not myPOTerm.Originated Then
                MessageBox.Show(txtCode.Text & " ไม่มีในระบบ")
                myPOTerm = oldPOTerm
            End If
            txtCode.Text = myPOTerm.Code
            txtName.Text = myPOTerm.Name
            If oldPOTerm.Id <> myPOTerm.Id Then
                oldPOTerm = myPOTerm
                Return True
            End If
            Return False
        ...
        Public Shared Sub ListInComboBox(ByVal cmb As ComboBox, Optional ByVal includeNothing As Boolean = False)
            ...
            For Each row As DataRow In dt.Rows
                Dim item As New IdValuePair(CInt(row("poterm_id")), CStr(row("poterm_name")))
                cmb.Items.Add(item)
            Next
