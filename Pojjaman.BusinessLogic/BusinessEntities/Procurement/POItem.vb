...
#Region "Methods"
    ...
    Public Sub CopyFromPRItem(ByVal prItem As PRItem)
      ...
      If Not newPritem.WBSDistributeCollection Is Nothing Then
        ...
        RemoveHandler Me.WBSDistributeCollection.PropertyChanged, AddressOf Me.WBSChangedHandler
        AddHandler Me.WBSDistributeCollection.PropertyChanged, AddressOf Me.WBSChangedHandler
        ...
        Else
          ...
          Next
          Me.Qty = (Math.Max(newPritem.StockQty - (newPritem.WithdrawnQty + newPritem.OrderedQty), 0)) / newPritem.Conversion 'เพื่อให้การเซต ปริมาณมีผลที่การจัดสรรด้วย
        End If
    ...
    Public Sub CopyToDataRow(ByVal row As TreeRow)
      ...
      End Try
    End Sub
    Public Property GetReceivedQty() As Decimal
      Get
        Return m_receivedQty
      End Get
      Set(ByVal value As Decimal)
...
#Region "Class Methods"
    Public Function GetLessPRReceiveDateFromPoitemColl() As Nullable(Of Date)
      ...
      End If
    End Function
    Public Function GetTopMostPRReceiveDateFromPoitemColl() As Nullable(Of Date)
      Dim arrDate As New ArrayList
      For Each pitem As POItem In Me
        If Not pitem Is Nothing AndAlso Not pitem.Pritem Is Nothing AndAlso Not pitem.Pritem.Pr Is Nothing Then
          arrDate.Add(pitem.Pritem.Pr.ReceivingDate)
    ...
    Public Sub GetReceivedQty()
      Dim ds As DataSet = SqlHelper.ExecuteDataset(Me.m_po.ConnectionString _
                                                   , CommandType.StoredProcedure _
                                                       , "GetStockedQty" _
                                                        , New SqlParameter("@po_id", Me.m_po.Id) _
                                                          )

      Dim stockedqtydic As New Dictionary(Of String, Decimal)

      For Each row As DataRow In ds.Tables(0).Rows
        Dim drh As New DataRowHelper(row)
        Dim key As String = drh.GetValue(Of String)("poi_linenumber") & "|" & drh.GetValue(Of String)("poi_entity") & "|" & drh.GetValue(Of String)("poi_entitytype")

        If Not stockedqtydic.ContainsKey(key) Then
          stockedqtydic.Add(key, drh.GetValue(Of Decimal)("receivedQty"))
        End If

      Next

      For Each poi As POItem In Me
        Dim key As String = poi.LineNumber.ToString & "|" & poi.Entity.Id.ToString & "|" & poi.ItemType.Value.ToString

        If stockedqtydic.ContainsKey(key) Then
          poi.GetReceivedQty = stockedqtydic.Item(key)
        End If
      Next
