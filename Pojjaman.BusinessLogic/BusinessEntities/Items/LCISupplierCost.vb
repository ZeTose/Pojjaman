...
Namespace Longkong.Pojjaman.BusinessLogic
    Public Class LCISupplierCostLink
...
#Region "Shared"
        Public Shared Function GetSchemaTable() As TreeTable
            Dim myDatatable As New TreeTable("SupplierCost")
            myDatatable.Columns.Add(New DataColumn("lcis_linenumber", GetType(Integer)))
            myDatatable.Columns.Add(New DataColumn("lcis_supplier", GetType(Integer)))
            myDatatable.Columns.Add(New DataColumn("Code", GetType(String)))
            myDatatable.Columns.Add(New DataColumn("Button", GetType(String)))
            myDatatable.Columns.Add(New DataColumn("Name", GetType(String)))
            myDatatable.Columns.Add(New DataColumn("Contact", GetType(String)))
            ...
            myDatatable.Columns("lcis_supplier").Caption = myStringParserService.Parse("${res:Longkong.Pojjaman.Gui.Panels.MatSupDetail.DescriptionHeaderText}")
...
#Region "Methods"
        Public Overloads Overrides Function Save(ByVal currentUserId As Integer, ByVal conn As System.Data.SqlClient.SqlConnection, ByVal trans As System.Data.SqlClient.SqlTransaction) As SaveErrorException
            If Me.m_lci Is Nothing Then
                Return New SaveErrorException("xxxx")
            End If
            Dim da As New SqlDataAdapter("Select * from supplierlcicost where lcis_lci=" & Me.m_lci.Id, conn)
                ...
                With ds.Tables("supplierlcicost")
                    ...
                    For n As Integer = 0 To Me.MaxRowIndex
                        ...
                        If ValidateRow(itemRow) Then
                            i += 1
                            Dim item As New LCISupplierCostLinkItem
                            item.CopyFromDataRow(itemRow)
                            Dim dr As DataRow = .NewRow
                            dr("lcis_lci") = Me.m_lci.Id
                            dr("lcis_linenumber") = i 'item("pri_linenumber")
                            If item.Supplier.Originated Then
                                dr("lcis_supplier") = item.Supplier.Id
                            End If
...
#Region "Items"
        ...
        Private Sub LoadItems()
            ...
            , "GetLCISupplierCosts" _
            , New SqlParameter("@lci_id", Me.m_lci.Id) _
            )

            For Each row As DataRow In ds.Tables(0).Rows
                Dim item As New LCISupplierCostLinkItem(row, "")
                item.LCISupplierCostLink = Me
                Me.Add(item)
            Next
        End Sub
        Private Sub LoadItems(ByVal ds As System.Data.DataSet, ByVal aliasPrefix As String)
            For Each dr As DataRow In ds.Tables(1).Rows
                Dim item As New LCISupplierCostLinkItem(dr, aliasPrefix)
                item.LCISupplierCostLink = Me
                Me.Add(item)
            Next
        End Sub
        Public Sub AddBlankRow(ByVal count As Integer)
            ...
            Next
        End Sub
        Public Function Add(ByVal item As LCISupplierCostLinkItem) As TreeRow
            ...
            Return myRow
        End Function
        Public Function Insert(ByVal index As Integer, ByVal item As LCISupplierCostLinkItem) As TreeRow
...
#Region "TreeTable Handlers"
        ...
        Public Sub SetCost(ByVal e As DataColumnChangeEventArgs)
            ...
            m_updating = False
        End Sub
        Private Function DupSupplier(ByVal sup As Supplier) As Boolean
            For Each row As TreeRow In Me.m_itemTable.Childs
                If Not row.IsNull("lcis_supplier") AndAlso IsNumeric(row("lcis_supplier")) AndAlso CInt(row("lcis_supplier")) = sup.Id Then
                    Return True
                End If
            Next
            Return False
        ...
        Public Sub SetSupplierValue(ByVal e As System.Data.DataColumnChangeEventArgs)
            If m_updating Then
                Return
            End If
            Dim mySupplier As New Supplier(e.ProposedValue.ToString)
            m_updating = True
            Dim msgServ As IMessageService = CType(ServiceManager.Services.GetService(GetType(IMessageService)), IMessageService)
            If mySupplier.Originated Then
                If DupSupplier(mySupplier) Then
                    msgServ.ShowMessageFormatted("${res:Global.Error.AlreadyHasCode}", New String() {mySupplier.ListPanelTitle, e.ProposedValue.ToString})
                    e.ProposedValue = e.Row(e.Column)
                    m_updating = False
                    Return
                End If
                Dim myUnit As Unit
                myUnit = Me.m_lci.DefaultUnit
                If Not myUnit Is Nothing AndAlso myUnit.Valid Then
                    e.Row("lcis_unit") = myUnit.Id
                    e.Row("Unit") = myUnit.Name
                Else
                    e.Row("lcis_unit") = DBNull.Value
                    e.Row("Unit") = DBNull.Value
                End If
                e.Row("lcis_supplier") = mySupplier.Id
                e.ProposedValue = mySupplier.Code
                e.Row("Name") = mySupplier.Name
                e.Row("Contact") = mySupplier.Contact
                e.Row("Phone") = mySupplier.Phone
            End If
            m_updating = False
        End Sub
...
#End Region
    ...
    Public Class LCISupplierCostLinkItem
        ...
        Protected Sub Construct(ByVal dr As DataRow, ByVal aliasPrefix As String)
            With Me
                If dr.Table.Columns.Contains(aliasPrefix & "lcis_cost") AndAlso Not dr.IsNull(aliasPrefix & "lcis_cost") Then
                    .m_cost = CDec(dr(aliasPrefix & "lcis_cost"))
                End If
                ...
                If dr.Table.Columns.Contains(aliasPrefix & "supplier_id") AndAlso Not dr.IsNull(aliasPrefix & "supplier_id") Then
                    If Not dr.IsNull("supplier_id") Then
                        .m_supplier = New Supplier(dr, "")
                    End If
                Else
                    If dr.Table.Columns.Contains(aliasPrefix & "lcis_supplier") AndAlso Not dr.IsNull(aliasPrefix & "lcis_supplier") Then
                        .m_supplier = New Supplier(CInt(dr(aliasPrefix & "lcis_supplier")))
                    End If
                End If
            End With
        End Sub
...
#Region "Methods"
        Public Sub CopyToDataRow(ByVal row As TreeRow)
            If row Is Nothing Then
                Return
            End If
            Me.LCISupplierCostLink.IsInitialized = False
            row("lcis_linenumber") = Me.LineNumber
            If Not Me.Supplier Is Nothing Then
                row("lcis_supplier") = Me.Supplier.Id
                row("Code") = Me.Supplier.Code
                row("Name") = Me.Supplier.Name
                row("Contact") = Me.Supplier.Contact
                row("Phone") = Me.Supplier.Phone
            End If
            ...
            End If
            If Me.Cost <> 0 Then
                row("lcis_cost") = Configuration.FormatToString(Me.Cost, DigitConfig.UnitPrice)
            Else
                row("lcis_cost") = ""
            End If
            row("lcis_costdate") = Me.CostDate
            'If Me.CostDate.Equals(Date.MinValue) Then
            '    row("lcis_costdate") = ""
            'Else
            '    row("lcis_costdate") = Me.CostDate.ToString
            'End If
            ...
            Else
                row("lcis_leadtime") = ""
            End If
            Me.LCISupplierCostLink.IsInitialized = True
        End Sub
        Public Sub CopyFromDataRow(ByVal row As TreeRow)
            ...
            Try
                ...
                End If
                If Not row.IsNull(("lcis_supplier")) Then
                    Me.Supplier = New Supplier(CInt(row("lcis_supplier")))
                End If
