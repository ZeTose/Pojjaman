...
#Region "Shared"
        Public Shared Function GetSchemaTable() As TreeTable
            Dim myDatatable As New TreeTable("LaborCost")
...
#Region "Methods"
        Public Overloads Overrides Function Save(ByVal currentUserId As Integer, ByVal conn As System.Data.SqlClient.SqlConnection, ByVal trans As System.Data.SqlClient.SqlTransaction) As SaveErrorException
            ...
            End If
            Dim da As New SqlDataAdapter("Select * from lcilabor where lcil_lci=" & Me.m_lci.Id & " and lcil_type=1", conn)
...
#Region "Items"
        ...
        Private Sub LoadItems()
            ...
            , New SqlParameter("@lcil_type", "1") _
            )

            For Each row As DataRow In ds.Tables(0).Rows
                Dim item As New LCILaborCostLinkItem(row, "")
                item.LCILaborCostLink = Me
                Me.Add(item)
            Next
        End Sub
        Private Sub LoadItems(ByVal ds As System.Data.DataSet, ByVal aliasPrefix As String)
            For Each dr As DataRow In ds.Tables(1).Rows
                Dim item As New LCILaborCostLinkItem(dr, aliasPrefix)
                item.LCILaborCostLink = Me
                Me.Add(item)
            Next
        End Sub
        Public Sub AddBlankRow(ByVal count As Integer)
            ...
            Next
        End Sub
        Public Function Add(ByVal item As LCILaborCostLinkItem) As TreeRow
            ...
            Return myRow
        End Function
        Public Function Insert(ByVal index As Integer, ByVal item As LCILaborCostLinkItem) As TreeRow
...
#Region "TreeTable Handlers"
        ...
        Public Function ValidateRow(ByVal row As TreeRow) As Boolean
            ...
            If (IsDBNull(proposedCode) OrElse proposedCode.ToString.Length = 0) _
                Then
                Return False
            End If
            Return True
        ...
        Public Sub SetCostDate(ByVal e As DataColumnChangeEventArgs)
            ...
            msgServ.ShowMessage("${res:Global.Error.CannotChangeLaborCostDate}")
        ...
        Public Sub SetCost(ByVal e As DataColumnChangeEventArgs)
            ...
            m_updating = False
        End Sub
        Private Function DupLabor(ByVal lab As Labor) As Boolean
            For Each row As TreeRow In Me.m_itemTable.Childs
                If Not row.IsNull("lcil_labor") AndAlso IsNumeric(row("lcil_labor")) AndAlso CInt(row("lcil_labor")) = lab.Id Then
                    Return True
                End If
            Next
            Return False
        ...
        Public Sub SetLaborValue(ByVal e As System.Data.DataColumnChangeEventArgs)
            If m_updating Then
                Return
            End If
            Dim myLabor As New Labor(e.ProposedValue.ToString)
            m_updating = True
            Dim msgServ As IMessageService = CType(ServiceManager.Services.GetService(GetType(IMessageService)), IMessageService)
            If myLabor.Originated Then
                If DupLabor(myLabor) Then
                    msgServ.ShowMessageFormatted("${res:Global.Error.AlreadyHasCode}", New String() {myLabor.ListPanelTitle, e.ProposedValue.ToString})
                    e.ProposedValue = e.Row(e.Column)
                    m_updating = False
                    Return
                End If
                Dim myUnit As Unit = myLabor.Unit
                If myUnit Is Nothing OrElse Not myUnit.Originated Then
                    msgServ.ShowMessageFormatted("${res:Global.Error.NoLaborUnit}", New String() {e.ProposedValue.ToString})
                    e.ProposedValue = e.Row(e.Column)
                    m_updating = False
                    Return
                End If
                If Not Lci.ValidUnit(myUnit) Then
                    msgServ.ShowMessageFormatted("${res:Global.Error.LaborLCIUnitNotMatched}", New String() {myUnit.Name, e.ProposedValue.ToString, Me.Lci.Name})
                    e.ProposedValue = e.Row(e.Column)
                    m_updating = False
                    Return
                End If
                e.Row("Unit") = myUnit.Name
                e.Row("lcil_Labor") = myLabor.Id
                e.ProposedValue = myLabor.Code
                e.Row("Name") = myLabor.Name
                e.Row("Cost") = myLabor.Cost
                e.Row("CostDate") = myLabor.CostDate
            End If
            m_updating = False
        End Sub
...
#End Region
        ...
        Public Overrides ReadOnly Property ClassName() As String
            Get
                Return "LCILaborCostLink"
            End Get
        End Property
    End Class

    Public Class LCILaborCostLinkItem

#Region "Members"
        Private m_lciLaborCostLink As LCILaborCostLink
        Private m_lineNumber As Integer
        Private m_labor As Labor
...
#Region "Methods"
        Public Sub CopyToDataRow(ByVal row As TreeRow)
            If row Is Nothing Then
                Return
            End If
            Me.LCILaborCostLink.IsInitialized = False
            row("lcil_linenumber") = Me.LineNumber
            If Not Me.Labor Is Nothing Then
                row("lcil_labor") = Me.Labor.Id
                row("Code") = Me.Labor.Code
                row("Name") = Me.Labor.Name
                row("Cost") = Me.Labor.Cost
                If Not Me.Labor.Unit Is Nothing Then
                    row("Unit") = Me.Labor.Unit.Name
                End If
                row("CostDate") = Me.Labor.CostDate
            End If
            If row.IsNull("Cost") OrElse Not IsNumeric(row("Cost")) OrElse CDec(row("Cost")) = 0 Then
                row("Cost") = ""
            End If
            row("lcil_note") = Me.Note
            Me.LCILaborCostLink.IsInitialized = True
        End Sub
        Public Sub CopyFromDataRow(ByVal row As TreeRow)
            ...
            Try
                ...
                If Not row.IsNull(("lcil_labor")) Then
                    Me.Labor = New Labor(CInt(row("lcil_labor")))
                End If
