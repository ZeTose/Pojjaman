...
Namespace Longkong.Pojjaman.Gui.Panels
    Public Class BillIssueSelectionView
...
#Region " Windows Form Designer generated code "
        ...
        <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
            ...
            '
            'BillIssueSelectionView
            '
            Me.Controls.Add(Me.tgItem)
            Me.Controls.Add(Me.Splitter1)
            Me.Controls.Add(Me.pnlFilter)
            Me.Name = "BillIssueSelectionView"
...
#Region "Constructors"
        Public Sub New(ByVal entity As ISimpleEntity, ByVal handler As Object, ByVal basket As BasketDialog, ByVal filters As Filter(), ByVal entities As ArrayList)
            ...
            'Hack
			m_filterSubPanel = New BillIssueFilterSubPanel(True)
...
#Region "Methods"
        ...
        End Sub
        Private receives_id As Integer
        Public Sub SearchData(ByVal order As String)
            ...
            If otherLength > 0 Then
                ...
                Next
                receives_id = CInt(newfilters(newfilters.Length - 1).Value)
            End If

            Dim dt As TreeTable = Milestone.GetSelectionListDatatable(newfilters)
...
#Region "Style"
        Public Function CreateListTableStyle() As DataGridTableStyle
            ...
            dst.MappingName = "BillIssueItems"
      ...
      csMilestoneDate.ReadOnly = True
            ...
            Dim csCC As New TreeTextColumn
            csCC.MappingName = "CostCenterName"
            csCC.HeaderText = "CostCenter"
            csCC.NullText = ""
            csCC.Width = 150
            csCC.ReadOnly = True
                ...
                For Each row As TreeRow In myTable.Childs
                    For Each childRow As TreeRow In row.Childs
                        If Not IsDBNull(childRow("Selected")) Then
                            If CBool(childRow("Selected")) Then
                                Dim id As Integer = CInt(childRow("salebilli_id"))
                                Dim stockCode As String = CStr(childRow("Code"))
                                Dim fullClassName As String = "Longkong.Pojjaman.BusinessLogic.Milestone"
                                ...
                                sbi.Id = bi.Id
                                sbi.Code = bi.StockCode
                                Dim newItem As New SaleBillIssueItem
                                newItem.SaleBillIssue = sbi
                                If Not childRow.IsNull("stock_id") Then
                                    newItem.Id = CInt(childRow("stock_id"))
                                End If
                                newItem.Code = bi.EntityName
                                newItem.ParentCode = sbi.Code
                                newItem.ParentId = sbi.Id
                                newItem.ParentType = 81
                                ...
                                End If
                                If Not childRow.IsNull("salebillii_unreceivedamt") Then
                                    newItem.UnreceivedAmount = CDec(childRow("salebillii_unreceivedamt"))
                                End If
                                If Not childRow.IsNull("salebillii_billedamt") Then
                                    newItem.BilledAmount = CDec(childRow("salebillii_billedamt"))
                                End If
