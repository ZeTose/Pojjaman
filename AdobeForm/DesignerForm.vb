...
#Region "Constructors"
    ...
    Public Sub New(ByVal prinTableEntity As IPrintableEntity)
      ...
      If TypeOf m_entity Is IHasStatusString Then
        m_StatusText = CType(m_entity, IHasStatusString).StatusString
      End If
...
#Region "Methods"
    ...
    Protected Overridable Sub PrintPage_Handler(ByVal sender As Object, ByVal pe As PrintPageEventArgs)
      ...
      Try
        ...
        If Not String.IsNullOrEmpty(m_StatusText) Then
          '=========================DRAFT=====================
          Dim pen As New Pen(Color.Red)
          pen.Width = 1.5
          pen.DashStyle = DashStyle.Solid

          Dim xstf As New StringFormat
          xstf.Trimming = StringTrimming.None
          xstf.FormatFlags = StringFormatFlags.NoClip
          xstf.LineAlignment = StringAlignment.Center
          xstf.Alignment = StringAlignment.Center

          Dim rectW As Integer = AnyThingToPixel("20cm")
          Dim rectH As Integer = AnyThingToPixel("10cm")
          Dim marginW As Integer = pe.MarginBounds.Width
          Dim marginH As Integer = pe.MarginBounds.Height

          Dim rectLoc As New Point((marginW / 2) - (rectW / 2), (marginH / 2) - (rectH / 2))
          Dim rectSize As New Size(rectW, rectH)
          Dim interval As Integer = 6
          Dim rect As Rectangle = New Rectangle(rectLoc, rectSize)

          Dim gp As New GraphicsPath
          Dim myFont As New Font("Arial", 150, FontStyle.Regular, GraphicsUnit.Point, CType(222, Byte))
          gp.AddString(m_StatusText _
                       , myFont.FontFamily _
                       , CInt(myFont.Style) _
                       , myFont.Size _
                       , rect _
          ...
          Dim m As New Matrix
          m.RotateAt(-35, New Point(rect.X + (rectW / 2), rect.Y + (rectH / 2)))
          gp.Transform(m)
          g.DrawPath(pen, gp)
          '=========================DRAFT=====================
        End If
