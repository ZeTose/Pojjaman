...
#Region "Methods"
    ...
    Private Sub CreateHeader()
      ...
      m_grid.ColWidths(3) = 80 'วันที่เอกสาร'
      m_grid.ColWidths(4) = 120
      m_grid.ColWidths(5) = 120 'วันที่ถูกหัก ณ ที่จ่าย'
      m_grid.ColWidths(6) = 150
      m_grid.ColWidths(7) = 180
      m_grid.ColWidths(8) = 120 'เลขที่ GL'
      m_grid.ColWidths(9) = 80 'วันที่ GL'
      m_grid.ColWidths(10) = 120 'เลขประจำตัว'
      ...
      m_grid(0, 1).CellValue = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptOutgoinWht.Date}") '"วันที่ใบกำกับภาษี"
      ...
      m_grid(0, 3).CellValue = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptOutgoinWht.DocDate}") '"วันที่เอกสาร" ///
      m_grid(0, 4).CellValue = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptOutgoinWht.Invoice}") '"เลขที่หัก ณ ที่จ่าย"
      m_grid(0, 5).CellValue = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptOutgoinWht.WHTDate}") '"วันที่ถูกหัก ณ ที่จ่าย" ///
      m_grid(0, 6).CellValue = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptOutgoinWht.PrintName}") '"ผู้ถูกหัก ณ ที่จ่าย"
      m_grid(0, 7).CellValue = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptOutgoinWht.WHTType}") '"ประเภท"

      m_grid(0, 8).CellValue = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptOutgoinWht.GLCode}") '"เลขที่ GL" ///
      m_grid(0, 9).CellValue = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptOutgoinWht.GLDate}") '"วันที่ GL" ///
      m_grid(0, 10).CellValue = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptOutgoinWht.TaxID}") '"เลขประจำตัว" ///

      m_grid(1, 7).CellValue = indent & Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptOutgoinWht.Description}") '"รายการ"
      m_grid(1, 11).CellValue = indent & Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptOutgoinWht.BeforeTax}") '"จำนวนเงิน"
      m_grid(1, 12).CellValue = indent & Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptOutgoinWht.TaxRate}") '"อัตรา"
      m_grid(1, 13).CellValue = indent & Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptOutgoinWht.WHTAmt}") '"ถูกหัก ณ ที่จ่าย"
    ...
    Private Sub PopulateData()
      ...
      m_grid.RowStyles(currDocIndex).ReadOnly = True
      m_grid(currDocIndex, 7).CellValue = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptOutgoinWht.SumAll}")   '"รวมทั้งหมด"
