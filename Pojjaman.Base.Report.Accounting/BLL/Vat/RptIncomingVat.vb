...
#Region "Methods"
        ...
        Private Sub CreateHeader()
            ...
            m_grid(0, 1).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptIncomingVat.DocDate}") '"วันที่"
            m_grid(0, 2).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptIncomingVat.Invoice}") '"ใบกำกับภาษี"

            m_grid(0, 3).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptIncomingVat.SupplierName}") '"ชื่อผู้ขาย"
            m_grid(0, 4).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptIncomingVat.TaxId}") '"เลขประจำตัวผู้เสียภาษี"
            m_grid(0, 5).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptIncomingVat.BranchHead}") '"สำนักงานใหญ่"
            m_grid(0, 6).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptIncomingVat.BranchId}") '"สาขาที่"

            m_grid(0, 7).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptIncomingVat.BeforeTax}") '"มูลค่าสินค้า/บริการ"
            m_grid(0, 8).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptIncomingVat.TaxAmount}") '"จำนวนเงินภาษี"
            m_grid(0, 9).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptIncomingVat.AfterTax}") '"รวม"

            m_grid(0, 10).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptIncomingVat.TaxRate}") '"อัตราภาษี"
            m_grid(0, 11).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptIncomingVat.SubmitalDate}") '"วันที่ยื่นภาษี"
            m_grid(0, 12).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptIncomingVat.GroupName}") '"กลุ่มภาษี"


            m_grid(0, 13).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptCCPOSummary.CCName}") '"ชื่อ Cost Center"
            m_grid(0, 14).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptIncomingVat.DocCode}") '"เลขที่เอกสาร"
            m_grid(0, 15).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptIncomingVat.GLCode}") '"GLCode"
            m_grid(0, 16).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptIncomingVat.VatRunNumber}") '"เลขที่รัน"
            m_grid(0, 17).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptIncomingVat.RelatedDoc}") '"เลขที่เอกสารเกี่ยวข้อง"
        ...
        Private Sub PopulateData()
            ...
            m_grid.RowStyles(currDocIndex).ReadOnly = True
            m_grid(currDocIndex, 6).CellValue = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptIncomingVat.SumAll}") '"รวมทั้งหมด"
