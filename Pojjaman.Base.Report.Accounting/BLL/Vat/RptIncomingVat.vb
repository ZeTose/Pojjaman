...
#Region "Methods"
    ...
    Private Sub CreateHeader()
      ...
      m_grid(0, 2).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptIncomingVat.SubmitalDate}") '"วันที่ยื่นภาษี"
      m_grid(0, 3).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptIncomingVat.Invoice}") '"ใบกำกับภาษี"
      m_grid(0, 4).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptIncomingVat.DocCode}") '"เลขที่เอกสาร"
      m_grid(0, 5).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptCCPOSummary.CCName}") '"เลขที่เอกสาร"
      m_grid(0, 6).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptIncomingVat.VatRunNumber}") '"เลขที่รัน"
      m_grid(0, 7).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptIncomingVat.RelatedDoc}") '"เลขที่เอกสารเกี่ยวข้อง"
      m_grid(0, 8).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptIncomingVat.SupplierName}") '"ชื่อผู้ขาย"
      m_grid(0, 9).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptIncomingVat.GroupName}") '"กลุ่มภาษี"
    ...
    Private Sub PopulateData()
      ...
      For Each row As DataRow In dt.Rows
        ...
        If row("docId").ToString & row("vatrunnumber").ToString <> currentDocCode Then
          ...
          If Not row.IsNull("CostcenterName") Then
            m_grid(currDocIndex, 5).CellValue = row("CostcenterName").ToString
          End If
          ...
          If Not row.IsNull("Supplier") Then
            m_grid(currDocIndex, 8).CellValue = row("Supplier").ToString
          End If
          If Not row.IsNull("GroupName") Then
            m_grid(currDocIndex, 9).CellValue = row("GroupName").ToString
          End If
          If IsNumeric(row("BeforeTax")) Then
            m_grid(currDocIndex, 10).CellValue = Configuration.FormatToString(CDec(row("BeforeTax")), DigitConfig.Price)
            SumBeforeTax += Configuration.Format(CDec(row("BeforeTax")), DigitConfig.Price)
          End If
          If IsNumeric(row("TaxAmt")) Then
            m_grid(currDocIndex, 11).CellValue = Configuration.FormatToString(CDec(row("TaxAmt")), DigitConfig.Price)
      ...
      m_grid.RowStyles(currDocIndex).ReadOnly = True
      m_grid(currDocIndex, 9).CellValue = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptIncomingVat.SumAll}") '"รวมทั้งหมด"
      m_grid(currDocIndex, 10).CellValue = Configuration.FormatToString(SumBeforeTax, DigitConfig.Price)
      m_grid(currDocIndex, 11).CellValue = Configuration.FormatToString(SumTaxAmount, DigitConfig.Price)
      m_grid(currDocIndex, 12).CellValue = Configuration.FormatToString(SumAfterTax, DigitConfig.Price)
    End Sub
#End Region#Region "Shared"
