...
#Region "Methods"
        ...
        Private Sub CreateHeader()
            ...
            m_grid(1, 8).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptPObyLCI.Discamt}")  '"ส่วนลด"         
            m_grid(1, 9).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptPObyLCI.Amount}")  '"จำนวนเงิน"
        ...
        Private Sub PopulateData()
            ...
            Dim SumTotalDiscamt As Decimal = 0
            Dim tmpTotalDiscamt As Decimal = 0
            ...
            For Each row As DataRow In dt.Rows
                If row("ItemId").ToString & row("ItemCode").ToString & row("ItemName").ToString <> currentItemCode Then
                    ...
                    If SumItem > 1 Then
                        ...
                        End If
                        m_grid(currSupplierIndex, 8).CellValue = Configuration.FormatToString(tmpTotalDiscamt, DigitConfig.Price)
                ...
                If row("SupplierCode").ToString <> currentSupplierCode Then
                    ...
                    m_grid.RowCount += 1
                    currSupplierIndex = m_grid.RowCount
                    m_grid(currSupplierIndex, 0).Tag = row
                    m_grid.RowStyles(currSupplierIndex).ReadOnly = True
                    ...
                    If IsNumeric(row("Discamt")) Then
                        m_grid(currSupplierIndex, 8).CellValue = Configuration.FormatToString(CDec(row("Discamt")), DigitConfig.Price)
                        tmpTotalDiscamt += CDec(row("Discamt"))
                        SumTotalDiscamt += CDec(row("Discamt"))
                    Else
                        m_grid(currSupplierIndex, 8).CellValue = ""
                    End If
                    If IsNumeric(row("Amount")) Then
                        m_grid(currSupplierIndex, 9).CellValue = Configuration.FormatToString(CDec(row("Amount")), DigitConfig.Price)
                        tmpTotalAmount += CDec(row("Amount"))
                        SumTotalAmount += CDec(row("Amount"))
                    Else
                        m_grid(currSupplierIndex, 9).CellValue = ""
                    End If
                    tmpSupplier += 1
                    SumSupplier += 1
