...
#Region "Methods"
    ...
    Private Sub CreateHeader()
      ...
      m_grid(1, 3).Text = indent & Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptPOHistory.PriceUnit}")   '"ราคาต่อหน่วย"
      m_grid(1, 4).Text = indent & Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptPOHistory.Unit}")   '"หน่วย"
      m_grid(1, 5).Text = indent & Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptPOHistory.Num}")   '"จำนวน"
      m_grid(1, 6).Text = indent & Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptPOHistory.Price}")   '"ราคา"
      m_grid(1, 7).Text = indent & Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptPOHistory.ReceiveDate}")   '"วันที่รับของ"
      m_grid(1, 8).Text = indent & Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptPOHistory.ReciveCode}")   '"เลขที่รับของ"
      m_grid(1, 9).Text = indent & Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptPOHistory.PODate}")   '"วันที่สั่งซื้อ"
      m_grid(1, 10).Text = indent & Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptPOHistory.POCode}")   '"เลขที่ใบสั่งซื้อ"
      m_grid(1, 11).Text = indent & Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptPOHistory.Discount}")   '"ส่วนลดรายการ"
          ...
          If CDate(row("ReceiveDate")).ToShortDateString & row("ReceiveCode").ToString & row("UnitPrice").ToString <> currentDocCode Then
            If (topLatest = 0 Or n < topLatest) Then
              ...
              If IsNumeric(row("UnitPrice")) Then
                m_grid(currDocIndex, 3).CellValue = Configuration.FormatToString(CDec(row("UnitPrice")), DigitConfig.Price)
              End If
              If Not row.IsNull("Unit") Then
                m_grid(currDocIndex, 4).CellValue = indent & row("Unit").ToString
              End If
              If IsNumeric(row("Qty")) Then
                m_grid(currDocIndex, 5).CellValue = Configuration.FormatToString(CDec(row("Qty")), DigitConfig.Qty)
                tmpSumQty += CDec(row("Qty"))
              End If
              If IsNumeric(row("Price")) Then
                m_grid(currDocIndex, 6).CellValue = Configuration.FormatToString(CDec(row("Price")), DigitConfig.Price)
                tmpSumPrice += Configuration.FormatToString(CDec(row("Price")), DigitConfig.Price)
              End If
              If IsDate(row("ReceiveDate")) Then
                m_grid(currDocIndex, 7).CellValue = indent & CDate(row("ReceiveDate")).ToShortDateString
              End If
              If Not row.IsNull("ReceiveCode") Then
                m_grid(currDocIndex, 8).CellValue = indent & row("ReceiveCode").ToString
              End If
              If IsDate(row("PODate")) Then
                m_grid(currDocIndex, 9).CellValue = indent & CDate(row("PODate")).ToShortDateString
              End If
              If Not row.IsNull("POCode") Then
                m_grid(currDocIndex, 10).CellValue = indent & row("POCode").ToString
              End If
              If Not row.IsNull("ItemDiscount") Then
                m_grid(currDocIndex, 11).CellValue = Configuration.FormatToString(CDec(row("ItemDiscount")), DigitConfig.Price)
              End If
            End If
            n += 1
