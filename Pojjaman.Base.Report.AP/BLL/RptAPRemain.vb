        ...
        Private Sub PopulateData()
            ...
            For Each supplierRow As DataRow In dt.Rows
                ...
                If Not supplierRow.IsNull("OpeningBalance") Then
                    trSupplier("col2") = Configuration.FormatToString(CDec(supplierRow("OpeningBalance")), DigitConfig.Price)
                    sumOpenningBalance += CDec(supplierRow("OpeningBalance"))
                End If
                If Not supplierRow.IsNull("Amount") Then
                    If CDec(supplierRow("Amount")) <> 0 Then
                        trSupplier("col3") = Configuration.FormatToString(CDec(supplierRow("Amount")), DigitConfig.Price)
                        sumAmount += CDec(supplierRow("Amount"))
                    End If
                End If
                If Not supplierRow.IsNull("PCN") Then
                    If CDec(supplierRow("PCN")) <> 0 Then
                        trSupplier("col4") = Configuration.FormatToString(CDec(supplierRow("PCN")), DigitConfig.Price)
                        sumPCNAmount += CDec(supplierRow("PCN"))
                    End If
                End If
                If Not supplierRow.IsNull("Pays") Then
                    If CDec(supplierRow("Pays")) <> 0 Then
                        trSupplier("col5") = Configuration.FormatToString(CDec(supplierRow("Pays")), DigitConfig.Price)
                        sumPayAmount += CDec(supplierRow("Pays"))
                    End If
                End If
                If Not supplierRow.IsNull("EndingBalance") Then
                    trSupplier("col6") = Configuration.FormatToString(CDec(supplierRow("EndingBalance")), DigitConfig.Price)
                    sumEndingBalance += CDec(supplierRow("EndingBalance"))
                End If

                If Not supplierRow.IsNull("OpeningBalanceRetention") Then
                    trSupplier("col7") = Configuration.FormatToString(CDec(supplierRow("OpeningBalanceRetention")), DigitConfig.Price)
                ...
                If Not supplierRow.IsNull("Retention") Then
                    If CDec(supplierRow("Retention")) <> 0 Then
                        trSupplier("col8") = Configuration.FormatToString(CDec(supplierRow("Retention")), DigitConfig.Price)
                ...
                If Not supplierRow.IsNull("PayRetention") Then
                    If CDec(supplierRow("PayRetention")) <> 0 Then
                        trSupplier("col9") = Configuration.FormatToString(CDec(supplierRow("PayRetention")), DigitConfig.Price)
                        sumPaysRetention += CDec(supplierRow("PayRetention"))
                    End If
                End If
                If Not supplierRow.IsNull("EndingBalanceRetention") Then
                    trSupplier("col10") = Configuration.FormatToString(CDec(supplierRow("EndingBalanceRetention")), DigitConfig.Price)
                    ...
                    For Each detailRow As DataRow In dt2.Select("Supplier=" & supplierRow("Supplier_ID").ToString)
                        If Not trSupplier Is Nothing Then
                            ...
                            If Not detailRow.IsNull("OpeningBalance") Then
                                trDetail("col2") = Configuration.FormatToString(CDec(detailRow("OpeningBalance")), DigitConfig.Price)
                            End If
                            If Not detailRow.IsNull("Amount") Then
                                If CDec(detailRow("Amount")) <> 0 Then
                                    trDetail("col3") = Configuration.FormatToString(CDec(detailRow("Amount")), DigitConfig.Price)
                                End If
                            End If
                            If Not detailRow.IsNull("PCN") Then
                                If CDec(detailRow("PCN")) <> 0 Then
                                    trDetail("col4") = Configuration.FormatToString(CDec(detailRow("PCN")), DigitConfig.Price)
                                End If
                            End If
                            If Not detailRow.IsNull("Pays") Then
                                If CDec(detailRow("Pays")) <> 0 Then
                                    trDetail("col5") = Configuration.FormatToString(CDec(detailRow("Pays")), DigitConfig.Price)
                                End If
                            End If
                            If Not detailRow.IsNull("EndingBalance") Then
                                trDetail("col6") = Configuration.FormatToString(CDec(detailRow("EndingBalance")), DigitConfig.Price)
                            End If
                            If Not detailRow.IsNull("OpeningBalanceRetention") Then
                                trDetail("col7") = Configuration.FormatToString(CDec(detailRow("OpeningBalanceRetention")), DigitConfig.Price)
                            End If
                            If Not detailRow.IsNull("Retention") Then
                                If CDec(detailRow("Retention")) <> 0 Then
                                    trDetail("col8") = Configuration.FormatToString(CDec(detailRow("Retention")), DigitConfig.Price)
                                End If
                            End If
                            If Not detailRow.IsNull("PayRetention") Then
                                If CDec(detailRow("PayRetention")) <> 0 Then
                                    trDetail("col9") = Configuration.FormatToString(CDec(detailRow("PayRetention")), DigitConfig.Price)
                                End If
                            End If
                        End If
                    Next
            ...
            trSupplier("col2") = Configuration.FormatToString(sumOpenningBalance, DigitConfig.Price)
            If sumAmount > 0 Then
                trSupplier("col3") = Configuration.FormatToString(sumAmount, DigitConfig.Price)
            End If
            If sumPCNAmount > 0 Then
                trSupplier("col4") = Configuration.FormatToString(sumPCNAmount, DigitConfig.Price)
            End If
            If sumPayAmount > 0 Then
                trSupplier("col5") = Configuration.FormatToString(sumPayAmount, DigitConfig.Price)
            End If
            trSupplier("col6") = Configuration.FormatToString(sumEndingBalance, DigitConfig.Price)

            trSupplier("col7") = Configuration.FormatToString(sumOpeningBalanceRetention, DigitConfig.Price)
            If sumPCNAmount > 0 Then
                trSupplier("col8") = Configuration.FormatToString(sumRetention, DigitConfig.Price)
            End If
            If sumPayAmount > 0 Then
                trSupplier("col9") = Configuration.FormatToString(sumPaysRetention, DigitConfig.Price)
            End If
            trSupplier("col10") = Configuration.FormatToString(sumEndingBalanceRetention, DigitConfig.Price)
