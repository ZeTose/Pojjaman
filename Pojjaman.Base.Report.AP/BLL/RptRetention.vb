...
#Region "Methods"
        ...
        Private Sub PopulateData()
            ...
            Dim DicPaysRet As New Dictionary(Of String, List(Of PaysRet))
            Dim DocPaysRet As PaysRet
            Dim drh As DataRowHelper
            Dim key As String

            For Each drowRelRet As DataRow In dtRelRet.Rows

                drh = New DataRowHelper(drowRelRet)

                key = drh.GetValue(Of Decimal)("stock_id").ToString + "|" + drh.GetValue(Of Decimal)("retentiontype").ToString

                If Not DicPaysRet.ContainsKey(key) Then
                    DicPaysRet.Add(key, New List(Of PaysRet))
                    DocPaysRet = New PaysRet(drh)
                    DicPaysRet(key).Add(DocPaysRet)
                Else
                    DocPaysRet = New PaysRet(drh)
                    DicPaysRet(key).Add(DocPaysRet)
                End If
            ...
            For Each drowOnRet As DataRow In dtOnRet.Rows
                Try
                    ...
                    tmpPaysBalance = tmpOpbRetention + tmpRetention
                    ...
                    key = drowOnRet("Stock_id").ToString + "|" + drowOnRet("Stock_type").ToString
                    If DicPaysRet.ContainsKey(key) Then
                        i = 0
                        For Each payR As PaysRet In DicPaysRet(key)

                            i = i + 1

                            If i = 1 Then

                                If payR.gross > 0 Then
                                    trDoc(7) = Configuration.FormatToString(payR.gross, DigitConfig.Price)
                                    sumRetentionPays_Supplier += payR.gross
                                    sumRetentionPays_Costcenter += payR.gross
                                    sumRetentionPays += payR.gross
                                End If

                                tmpPaysBalance = tmpPaysBalance - payR.gross
                                ...
                                If payR.paysdate.Length > 0 Then
                                    trDoc(9) = payR.paysdate
                                End If

                                If payR.payscode.Length > 0 Then
                                    trDoc(10) = payR.payscode
                                End If

                                If payR.paymentcode.Length > 0 Then
                                    trDoc(11) = payR.paymentcode
                                End If
