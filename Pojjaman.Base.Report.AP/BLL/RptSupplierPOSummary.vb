...
#Region "Methods"
    ...
    Private Sub CreateHeader()
      ...
      m_grid.ColWidths(1) = 115
      m_grid.ColWidths(2) = 225
      m_grid.ColWidths(3) = 125
      m_grid.ColWidths(4) = 125
      m_grid.ColWidths(5) = 95
      m_grid.ColWidths(6) = 85
      m_grid.ColWidths(7) = 95
      m_grid.ColWidths(8) = 85
      m_grid.ColWidths(9) = 95
      m_grid.ColWidths(10) = 85
      ...
      m_grid(1, 5).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptSupplierPOSummary.ReceiveDate}")  '"วันที่รับของ"
      ...
      m_grid(1, 12).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptSupplierPOSummary.PONote}")  '"หมายเหตุ PO"
      ...
      m_grid(2, 4).Text = indent & Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptSupplierPOSummary.UnitName}") '"หน่วย"
      ...
      For Each row As DataRow In dt.Rows
        If row("SupplierId").ToString <> currentSupplierId Then
          If currentSupplierId <> "" Then
            ...
            m_grid(currItemIndex, 10).CellValue = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptSupplierPOSummary.Amount}") '"รวมเป็นเงิน"
            m_grid(currItemIndex, 11).CellValue = Configuration.FormatToString(tmpAfterTax, DigitConfig.Price)
          End If
          m_grid.RowCount += 1
        ...
        If row("DocId").ToString <> currentDocId Then
          ...
          If Not row.IsNull("EmployeeName") Then
            m_grid(currDocIndex, 3).CellValue = row("EmployeeName").ToString
          ...
          End If
          If Not row.IsNull("PO_Note") Then
            m_grid(currDocIndex, 12).CellValue = row("PO_Note")
          End If
...
#End Region#Region "IPrintableEntity"
    ...
    Public Overrides Function GetDocPrintingEntries() As DocPrintingItemCollection
      ...
      For rowIndex As Integer = 3 To m_grid.RowCount
        ...
        dpi = New DocPrintingItem
        dpi.Mapping = "col11"
        dpi.Value = CStr(m_grid(rowIndex, 12).CellValue)
        dpi.DataType = "System.String"
        dpi.Row = n + 1
        dpi.Table = "Item"
        If Not m_grid(rowIndex, 1).Tag Is Nothing Then
        ...
        'dpi = New DocPrintingItem
        'dpi.Mapping = "col12"
        'dpi.Value = CStr(m_grid(rowIndex, 13).CellValue)
        'dpi.DataType = "System.String"
        'dpi.Row = n + 1
        'dpi.Table = "Item"
        'If Not m_grid(rowIndex, 1).Tag Is Nothing Then
