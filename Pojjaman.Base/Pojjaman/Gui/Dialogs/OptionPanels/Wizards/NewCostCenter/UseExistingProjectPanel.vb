...
Namespace Longkong.Pojjaman.Gui.Dialogs.OptionPanels.NewCostCenterWizard
    Public Class UseExistingProjectPanel
...
#Region "Methods"
        Public Sub SetLabelText()
            Me.rdExist.Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.Gui.Dialogs.OptionPanels.NewCostCenterWizard.UseExistingProjectPanel.rdExist}")
            Me.rdNotExist.Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.Gui.Dialogs.OptionPanels.NewCostCenterWizard.UseExistingProjectPanel.rdNotExist}")
            txtMessage.Lines = Me.StringParserService.Parse("${res:Dialog.Wizards.NewCostCenterWizard.UseExistingProjectPanel.DescriptionText}").Split(New Char() {ChrW(10)})
        End Sub
        Private Sub RDCheck(ByVal sender As Object, ByVal e As EventArgs)
            If Me.rdExist.Checked Then
                Me.NextWizardPanelID = "ChooseProjectPanel"
            Else
                Me.NextWizardPanelID = "GoOnWithoutBOQPanel"
            End If
        End Sub
#End Region
                ...
                Case DialogMessage.Next
                    If Not Me.CostCenter Is Nothing Then
                        If Me.rdNotExist.Checked Then
                            Me.CostCenter.Project = New Project
                        End If
                    End If
