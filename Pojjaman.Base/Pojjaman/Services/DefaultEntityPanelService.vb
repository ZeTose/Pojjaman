...
#Region "GetView"
    ...
    , Optional ByVal isPanel As Boolean = False) As Object
      ...
      ElseIf TypeOf entity Is IUseXtrasReport Then
        Return New XtrasReportPanelView(entity, handler, New BasketDialog, filters, entities)
...
#Region "Open Panel"
    ...
    End Function
    Public Overloads Function OpenPanel(ByVal entityName As String, ByVal args As String, ByVal label As String) As IWorkbenchWindow Implements IEntityPanelService.OpenPanel
      Return OpenPanel(SimpleBusinessEntityBase.GetEntity(entityName), args, label)
    End Function
    Public Overloads Function OpenPanel(ByVal entity As ISimpleEntity) As IWorkbenchWindow Implements IEntityPanelService.OpenPanel
      Return OpenPanel(entity, "", "")
    End Function
    ''' <summary>
    ''' 
    ''' </summary>
    ''' <param name="args">คั่นด้วย | ในแต่ละ filter และคั่นด้วย : ระหว่างชื่อกับ value</param>
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Function GetFiltersFromArgs(ByVal args As String) As Filter()
      If args Is Nothing Then
        Return Nothing
      End If
      Dim ret As New List(Of Filter)
      Dim reg As New Regex("\|([^\|]+:[^\|]+)\|")
      For Each m As Match In reg.Matches(args)
        Dim sp As String() = m.Groups(1).Value.Split(":"c)
        Dim f As New Filter(sp(0), sp(1))
        ret.Add(f)
      Next
      Return ret.ToArray()
    End Function
    ...
    ''' เอาไว้เปิด entity
    ''' </summary>
    ''' <param name="entity">entity ที่ต้องการเปิด</param>
    ''' <param name="args">filter</param>
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Overloads Function OpenPanel(ByVal entity As ISimpleEntity, ByVal args As String, ByVal label As String) As IWorkbenchWindow
      If Me.IsOpen(entity) Then
        Dim view As IViewContent = WorkbenchSingleton.Workbench.GetView(entity)
        view.WorkbenchWindow.SelectWindow()
        Return view.WorkbenchWindow
      End If
      Dim filters() As Filter = GetFiltersFromArgs(args)
      Dim myView As Object = GetView(entity, New NamedEntityOperationDelegate(AddressOf Dummy), filters, Nothing, True)
      ...
      If Not String.IsNullOrEmpty(label) Then
        entity.MenuLabel = label
        myContent.ForceLabel = label
      End If
