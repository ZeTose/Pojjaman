...
#End Region
  ...
  '      Dim window As IWorkbenchWindow = WorkbenchSingleton.Workbench.ActiveWorkbenchWindow
  '      If ((Not window Is Nothing) AndAlso Not window.ViewContent.IsViewOnly) Then
  '        If TypeOf window.ViewContent Is Longkong.Pojjaman.Gui.Panels.ISimpleListPanelCanCopy Then
  '          Dim myEntityUtilityService As EntityUtilityService = CType(ServiceManager.Services.GetService(GetType(EntityUtilityService)), EntityUtilityService)
  '          myEntityUtilityService.ObservedSave(New EntityOperationDelegate(AddressOf window.ViewContent.Save), CType(window.ViewContent, ISimpleListPanelCanCopy).SelectedEntity)
  '          If IsEnabledWithChecking Then
  '            Dim panel As ISimpleListPanelCanCopy = CType(WorkbenchSingleton.Workbench.ActiveWorkbenchWindow.ViewContent, ISimpleListPanelCanCopy)
  '            panel.AddCopy()
  '          End If
  '        End If
  '      End If
...
#Region "Methods"
    Public Overrides Sub Run()
      ...
      If ((Not window Is Nothing) AndAlso Not window.ViewContent.IsViewOnly) Then
        ...
        End If
        '------------------------------------------
        'If TypeOf window.ViewContent Is GenericPanel Then
        '  window.ViewContent.Save()
        '  Return
        'End If
        '------------------------------------------
        If TypeOf window.ViewContent Is ISaveContent Then
          window.ViewContent.Save()
          Return
        End If
