...
#Region "IListDetail"
    Public Overrides Sub CheckFormEnable()
      ...
      ElseIf TypeOf (Me.m_entity) Is GoodsReceipt Then
        ...
        Else
          If (CType(Me.m_entity, GoodsReceipt).Locking) _
                    OrElse (CType(Me.m_entity, GoodsReceipt).Status.Value = 0 _
                    OrElse CType(Me.m_entity, GoodsReceipt).IsReferenced _
                    OrElse CType(Me.m_entity, GoodsReceipt).Payment.Status.Value = 0 _
                    OrElse CType(Me.m_entity, GoodsReceipt).Payment.Status.Value >= 3) _
            Then

            If CType(Me.m_entity, GoodsReceipt).IsReferenced AndAlso (CType(Me.m_entity, GoodsReceipt).Unlock = UnlockType.UnlockAll OrElse CType(Me.m_entity, GoodsReceipt).Unlock = UnlockType.UnlockLevel1) Then
              'Enable All
            Else
