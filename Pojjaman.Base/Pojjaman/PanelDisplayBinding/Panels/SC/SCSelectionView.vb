...
#Region "Members"
        ...
        Private m_scoposedBasketItems As BasketItemCollection
...
#Region "Constructors"
        Public Sub New(ByVal entity As ISimpleEntity, ByVal mode As Selection, ByVal basket As BasketDialog, ByVal filters As Filter(), ByVal entities As ArrayList)
            ...
            m_scoposedBasketItems = New BasketItemCollection
...
#Region "Methods"
        ...
        Public Sub SearchData(ByVal order As String)
            ...
            Dim filterdt As TreeTable = SCItem.GetListDatatable(newfilters)

            Dim dt As TreeTable = GetSchemaTable()
            dt.Clear()
            Dim parRow As TreeRow
            Dim currentSC As Integer = -1
            For Each filteredRow As TreeRow In filterdt.Rows
                Dim scId As Integer
                If Not filteredRow.IsNull("m_sc") Then
                    scId = CInt(filteredRow("m_sc"))
                End If
                Dim scRequestor As String
                If Not filteredRow.IsNull("Requestor") Then
                    scRequestor = CStr(filteredRow("Requestor"))
                End If
                ...
                If Not filteredRow.IsNull("Code") Then
                    scCode = CStr(filteredRow("Code"))
                End If

                If currentSC <> scId Then
                    parRow = dt.Childs.Add
                    parRow("Selected") = False
                    parRow("Material") = scCode
                    parRow("Qty") = ccInfo
                    parRow("Code") = scCode
                    parRow("Date") = scDate
                    ...
                    For Each childRow As TreeRow In row.Childs
                        If Not IsDBNull(childRow("Selected")) Then
                            If CBool(childRow("Selected")) Then
                                ...
                                Dim fullClassName As String = "Longkong.Pojjaman.BusinessLogic.SCItem"
                                ...
                                If TypeOf childRow.Tag Is SCItem Then
                                    Dim sci As SCItem = CType(childRow.Tag, SCItem)
                                    Dim theSC As SC = sci.SC
                                    sci = New SCItem(sci.SC.Id, sci.Linenumber)
                                    sci.Sc = theSC
                                    id = theSC.Id
