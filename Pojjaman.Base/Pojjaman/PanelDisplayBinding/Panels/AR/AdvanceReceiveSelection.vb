...
Namespace Longkong.Pojjaman.Gui.Panels
  Public Class AdvanceReceiveSelection
...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Protected Sub InitializeComponent()
      ...
      Dim resources As System.Resources.ResourceManager = New System.Resources.ResourceManager(GetType(AdvanceReceiveSelection))
      ...
      '
      'AdvanceReceiveSelection
      '
      Me.Controls.Add(Me.grbDetail)
      Me.Name = "AdvanceReceiveSelection"
...
#Region "Members"
    ...
    Private m_entity As IAdvanceReceiveItemAble
    Private m_collection As AdvanceReceiveItemCollection
    Private m_oldcollection As AdvanceReceiveItemCollection
#End Region

#Region "Constructors"
    Public Sub New(ByVal myEntity As IAdvanceReceiveItemAble)
      MyBase.New()
      Me.InitializeComponent()
      Me.SetLabelText()
      Initialize()

      Dim dt As TreeTable = AdvanceReceive.GetAdvanceReceiveSelectionSchema
      ...
      m_entity = myEntity
      m_collection = m_entity.AdvanceReceiveItemCollection
      m_oldcollection = CType(Me.m_collection.Clone, AdvanceReceiveItemCollection)
...
#Region "Style"
    Public Function CreateTableStyle() As DataGridTableStyle
      ...
      dst.MappingName = "AdvanceReceiveSelection"
      Dim myStringParserService As StringParserService = CType(ServiceManager.Services.GetService(GetType(StringParserService)), StringParserService)

      Dim csLineNumber As New TreeTextColumn
      csLineNumber.MappingName = "Linenumber"
      csLineNumber.HeaderText = myStringParserService.Parse("${res:Longkong.Pojjaman.Gui.Panels.AdvanceReceiveSelection.LineNumberHeaderText}")
      ...
      csCode.HeaderText = myStringParserService.Parse("${res:Longkong.Pojjaman.Gui.Panels.AdvanceReceiveSelection.CodeHeaderText}")
      csCode.NullText = ""
      csCode.Width = 80
      ...
      csAmount.HeaderText = myStringParserService.Parse("${res:Longkong.Pojjaman.Gui.Panels.AdvanceReceiveSelection.AmountHeaderText}")
      csAmount.NullText = ""
      csAmount.Width = 100
...
#Region "Properties"
    Private ReadOnly Property CurrentItem() As AdvanceReceiveItem
      Get
        Dim row As TreeRow = Me.m_treeManager.SelectedRow
        If row Is Nothing Then
          Return Nothing
        End If
        If Not TypeOf row.Tag Is AdvanceReceiveItem Then
          Return Nothing
        End If
        Return CType(row.Tag, AdvanceReceiveItem)
      End Get
    End Property
#End Region
...
#Region "TreeTable Handlers"
    ...
    Private Sub Treetable_ColumnChanging(ByVal sender As Object, ByVal e As System.Data.DataColumnChangeEventArgs)
      ...
      If Me.CurrentItem Is Nothing Then
        Dim doc As New AdvanceReceiveItem
      ...
      End Try
    End Sub
    Public Function ValidateRow(ByVal e As DataColumnChangeEventArgs) As Boolean
      ...
      If IsDBNull(amount) OrElse Not IsNumeric(amount) OrElse CDec(amount) <= 0 Then 'OrElse CDec(amount) > CDec(remain) Then
        e.Row.SetColumnError("amount", Me.StringParserService.Parse("${res:Global.Error.AdvanceReceiveItemMissing}"))
      Else
    ...
    Public Sub SetAmount(ByVal e As DataColumnChangeEventArgs)
      If m_updating Then
        Return
      End If
      Dim doc As AdvanceReceiveItem = Me.CurrentItem
      ...
      If IsNumeric(e.Row(e.Column)) Then
        remaining = CDec(e.ProposedValue)
      End If
      Dim msgServ As IMessageService = CType(ServiceManager.Services.GetService(GetType(IMessageService)), IMessageService)
      If doc.AdvanceReceive.RemainingAmount < remaining Then
        msgServ.ShowError("${res:Global.Error.noBalanceRemaining}")
    ...
    Private Function DupCode(ByVal e As DataColumnChangeEventArgs) As Boolean
      ...
      If doc Is Nothing Then
        Return False
      End If
      For Each item As AdvanceReceiveItem In Me.m_collection
        If Not doc Is item Then
          If e.ProposedValue.ToString.ToLower = item.AdvanceReceive.Code.ToLower Then
            Return True
          End If
        End If
      Next
    ...
    Public Sub SetCode(ByVal e As DataColumnChangeEventArgs)
      ...
      If DupCode(e) Then
        ...
        Return
      End If
      Dim advpay As New AdvanceReceive(e.ProposedValue.ToString)
            'If (m_entity.TaxType.Value = 0 AndAlso advpay.TaxType.Value = 0) OrElse _
            '   (m_entity.TaxType.Value > 0 AndAlso advpay.TaxType.Value = 2) Then
            '  'nothing
            'Else
            '  msgServ.ShowMessageFormatted("${res:Global.Error.ValidTaxTypeAdvanceReceive}", New String() {e.ProposedValue.ToString})
            '  e.ProposedValue = e.Row(e.Column)
            '  m_updating = False
            '  Return
            'End If
...
#Region "IListDetail"
    Public Overrides Sub CheckFormEnable()
      ...
      ElseIf TypeOf (Me.m_entity) Is GoodsSold Then
        If CType(Me.m_entity, GoodsSold).Status.Value = 0 _
         OrElse CType(Me.m_entity, GoodsSold).IsReferenced = True _
        OrElse CType(Me.m_entity, GoodsSold).Receive.Status.Value = 0 _
        OrElse CType(Me.m_entity, GoodsSold).Receive.Status.Value >= 3 Then
    ...
    Public Overrides Sub SetLabelText()
      ...
      Me.grbDetail.Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.Gui.Panels.AdvanceReceiveSelection.grbDetail}")
      Me.lblGross.Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.Gui.Panels.AdvanceReceiveSelection.lblGross}")
...
#Region "Event Handlers"
    Public Sub ItemButtonClick(ByVal e As ButtonColumnEventArgs)
      ...
      entity.Add(m_entity.Customer)

			Dim filters(1) As Filter
      filters(0) = New Filter("IDList", GenIDList())
      'filters(1) = New Filter("showOnlyAmountMoreThanZero", True)
            'filters(1) = New Filter("TaxType", m_entity.TaxType.Value)
			filters(1) = New Filter("showOnlyAmountMoreThanZero", 1)
      Dim myEntityPanelService As IEntityPanelService = CType(ServiceManager.Services.GetService(GetType(IEntityPanelService)), IEntityPanelService)
      myEntityPanelService.OpenListDialog(New AdvanceReceive, AddressOf SetAdvanceReceive, filters, entity)
    End Sub
    Private Sub SetAdvanceReceive(ByVal items As BasketItemCollection)
      Dim index As Integer = tgItem.CurrentRowIndex
      For i As Integer = items.Count - 1 To 0 Step -1
        Dim item As BasketItem = CType(items(i), BasketItem)
        Dim myItem As New AdvanceReceiveItem
        myItem.AdvanceReceive = New AdvanceReceive(item.Id)
        myItem.Amount = myItem.AdvanceReceive.GetRemainingAmount
        If i = items.Count - 1 Then
          If m_collection.Count = 0 Then
            m_collection.Add(myItem)
          Else
            If Not Me.CurrentItem Is Nothing Then
              Me.CurrentItem.AdvanceReceive = myItem.AdvanceReceive
              Me.CurrentItem.Amount = myItem.AdvanceReceive.GetRemainingAmount
            Else
              m_collection.Add(myItem)
            End If
          End If
    ...
    Private Function GenIDList() As String
      ...
      For Each item As AdvanceReceiveItem In Me.m_collection
        If Not item.AdvanceReceive Is Nothing AndAlso item.AdvanceReceive.Originated Then
          idlist += CStr(item.AdvanceReceive.Id)
    ...
    Private Sub ibtnBlank_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ibtnBlank1.Click
      ...
      If index = -1 OrElse index > Me.m_collection.Count - 1 Then
        Me.m_collection.Add(New AdvanceReceiveItem)
      Else
        Me.m_collection.Insert(index, New AdvanceReceiveItem)
      End If
      RefreshDocs()
      tgItem.CurrentRowIndex = index
    End Sub
    Private Sub ibtnDelRow_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ibtnDelRow1.Click
      ...
      If row Is Nothing Then
        Return
      End If
      Dim item As AdvanceReceiveItem = Me.CurrentItem
...
#Region "Buttons Event"
    ...
    Private Sub btnCancel_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnCancel1.Click
      Me.m_entity.AdvanceReceiveItemCollection = Me.m_oldcollection
    End Sub
#End Region
