...
Namespace Longkong.Pojjaman.Gui.Panels
  Public Class CostCenterPanelView
...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
      ...
      Dim resources As System.Resources.ResourceManager = New System.Resources.ResourceManager(GetType(CostCenterPanelView))
      ...
      'chkAllowDrag
      '
      Me.chkAllowDrag.Checked = False
      Me.chkAllowDrag.CheckState = System.Windows.Forms.CheckState.Unchecked
...
#Region "Constructors"
    ...
    Private Sub Construct(ByVal entity As TreeBaseEntity, ByVal mode As Selection, ByVal basket As BasketDialog, ByVal filters As Filter(), ByVal entities As ArrayList)
      ...
      Me.imlTree.Images.Add(myIConService.GetBitmap("Icons.16x16.HQ")) '0
      Me.imlTree.Images.Add(myIConService.GetBitmap("Icons.16x16.HQ.Cut")) '1
      Me.imlTree.Images.Add(myIConService.GetBitmap("Icons.16x16.Department")) '2
      Me.imlTree.Images.Add(myIConService.GetBitmap("Icons.16x16.Department.Cut")) '3
      Me.imlTree.Images.Add(myIConService.GetBitmap("Icons.16x16.Site")) '4
      Me.imlTree.Images.Add(myIConService.GetBitmap("Icons.16x16.Site.Cut")) '5
    End Sub
#End Region
    ...
    Private Sub tvGroup_DragDrop(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DragEventArgs) Handles tvGroup.DragDrop
      ...
      Try
        If Not targetNode.Bounds.Contains(pt) Then
          ...
          End If
          Dim myentity As TreeBaseEntity = CType(SimpleBusinessEntityBase.GetEntity(Me.Entity.FullClassName, CInt(GetIDFromTag(sourceNode.Tag))), TreeBaseEntity)
    ...
    Private Sub tvGroup_AfterSelect(ByVal sender As System.Object, ByVal e As System.Windows.Forms.TreeViewEventArgs) Handles tvGroup.AfterSelect
      ...
      If Not e.Node Is Nothing Then
        If TypeOf e.Node.Tag Is TreeBaseEntity Then
          ...
          End If
        Else
          Me.m_selectedEntity = CType(SimpleBusinessEntityBase.GetEntity(Me.Entity.FullClassName, CInt(GetIDFromTag(e.Node.Tag))), TreeBaseEntity)
          If Not Me.m_selectedEntity Is Nothing Then
            AddHandler Me.m_selectedEntity.TabPageTextChanged, AddressOf Me.ChangeTitle
          End If
        End If
...
#Region "IGroupPanel"
    ...
    Public Sub RefreshData(ByVal id As String) Implements IGroupPanel.RefreshData
      ...
      If IsNumeric(id) Then
        Dim node As TreeNode = TreeViewHelper.SearchTagString(tvGroup, CInt(id))
    ...
    Public Sub RefreshData(ByVal entity As TreeBaseEntity) Implements IGroupPanel.RefreshData
      ...
      If entity Is Nothing OrElse Not entity.Originated Then
        ...
        Return
      End If
      If TreeViewHelper.SearchTagString(tvGroup, entity.Id) Is Nothing Then
        'เพิ่ง add เข้ามา
        Dim newNode As New TreeNode(entity.Code & " - " & entity.Name)
        newNode.Tag = entity.Id.ToString & "|" & CType(entity, CostCenter).Type.Value.ToString
        ...
        Else
          'มี parent
          Dim parnode As TreeNode = TreeViewHelper.SearchTagString(tvGroup, entity.Parent.Id)
        ...
        Return
      End If
      Dim node As TreeNode = TreeViewHelper.SearchTagString(tvGroup, CInt(entity.Id))
      If Not node Is Nothing Then
    ...
    Private Sub GetCheckNodes(ByVal nodes As TreeNodeCollection)
      For Each node As TreeNode In nodes
        If node.Checked Then
          Dim id As Integer = CInt(GetIDFromTag(node.Tag))
          checkIds.Add(id)
        End If
...
#Region "IClipboardHandler"
    Public Overrides Sub Copy(ByVal sender As Object, ByVal e As System.EventArgs)
      ...
      If oldData.GetDataPresent(Me.Entity.FullClassName) Then
        ...
        Dim oldCopiedNode As TreeNode = TreeViewHelper.SearchTagString(tvGroup, id)
        If Not oldCopiedNode Is Nothing Then
          oldCopiedNode.SelectedImageIndex = GetImageIndexFromTag(oldCopiedNode.Tag, False)
          oldCopiedNode.ImageIndex = oldCopiedNode.SelectedImageIndex
        End If
      End If

      Dim newData As New DataObject
      newData.SetData(Me.Entity.FullClassName, GetIDFromTag(tvGroup.SelectedNode.Tag))
    ...
    Public Overrides Sub Cut(ByVal sender As Object, ByVal e As System.EventArgs)
      ...
      Dim newData As New DataObject
      newData.SetData(Me.Entity.FullClassName, GetIDFromTag(tvGroup.SelectedNode.Tag))
      tvGroup.SelectedNode.SelectedImageIndex = GetImageIndexFromTag(tvGroup.SelectedNode.Tag, True)
      tvGroup.SelectedNode.ImageIndex = tvGroup.SelectedNode.SelectedImageIndex
      Clipboard.SetDataObject(newData)
      Me.RefreshEditableStatus()
    End Sub
    ...
    Public Overrides ReadOnly Property EnablePaste() As Boolean
      Get
        ...
        If data.GetDataPresent(Me.Entity.FullClassName) Then
          ...
          Dim copiedNode As TreeNode = TreeViewHelper.SearchTagString(tvGroup, id)
          If Not copiedNode Is Nothing AndAlso (copiedNode.ImageIndex Mod 2) <> 0 AndAlso Not tvGroup.SelectedNode Is Nothing Then
            Return True
          End If
        End If
    ...
    Public Overrides Sub Paste(ByVal sender As Object, ByVal e As System.EventArgs)
      ...
      If data.GetDataPresent(Me.Entity.FullClassName) Then
        Dim id As Integer = CInt(data.GetData(Me.Entity.FullClassName))
        Dim copiedNode As TreeNode = TreeViewHelper.SearchTagString(tvGroup, id)
        If (copiedNode.ImageIndex Mod 2) <> 0 Then
          TryPaste(copiedNode, tvGroup.SelectedNode)
          copiedNode.ImageIndex = GetImageIndexFromTag(copiedNode.Tag, False)
          copiedNode.SelectedImageIndex = copiedNode.ImageIndex
...
#End Region
    ...
    Private Function GetIDFromTag(ByVal tag As Object) As Integer
      If tag Is Nothing Then
        Return -1
      End If
      If tag.ToString.IndexOf("|"c) > 0 Then ' > 0 เพราะต้องมีอย่างอื่นอยู่หน้า |
        Return CInt(tag.ToString.Split("|"c)(0))
      End If
    End Function
    Private Function GetImageIndexFromTag(ByVal tag As Object, ByVal isCut As Boolean) As Integer
      If tag Is Nothing Then
        Return -1
      End If
      If tag.ToString.IndexOf("|"c) > 0 Then ' > 0 เพราะต้องมีอย่างอื่นอยู่หน้า |
        Dim type As Integer = CInt(tag.ToString.Split("|"c)(1))
        Select Case type
          Case 0
            If Not isCut Then
              Return 0
