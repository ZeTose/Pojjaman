...
#Region "Methods"
    ...
    Private Sub PopulateData()
      ...
      Dim CustHaveTran As New List(Of String)
      ...
      For Each row As DataRow In dt2.Rows
        If row("CustomerCode").ToString <> currentCustomerCode Then
          ...
          CustHaveTran.Add(currentCustomerCode)

          LastCustomerCrAmtRecord = CDec(row("CreditAmount"))
          Dim supRows As DataRow() = dt.Select("CustomerCode ='" & currentCustomerCode & "'")
      ...
      'เอาลูกค้าที่ไม่มีการเคลื่อนไหวแสดงด้วย
      For Each crow As DataRow In dt.Rows
        Dim crh As New DataRowHelper(crow)
        If Not CustHaveTran.Contains(crh.GetValue(Of String)("CustomerCode")) Then
          m_grid.RowCount += 1
          currentCustomerIndex = m_grid.RowCount
          m_grid.RowStyles(currentCustomerIndex).BackColor = Color.FromArgb(128, 255, 128)
          m_grid.RowStyles(currentCustomerIndex).Font.Bold = True
          ...
          m_grid(currentCustomerIndex, 1).CellValue = crh.GetValue(Of String)("CustomerCode")
          m_grid(currentCustomerIndex, 2).CellValue = crh.GetValue(Of String)("CustomerName")
          m_grid(currentCustomerIndex, 3).CellValue = Configuration.FormatToString(crh.GetValue(Of Decimal)("CreditAmount") - crh.GetValue(Of Decimal)("OpenningBalance"), DigitConfig.Price)
          m_grid(currentCustomerIndex, 3).HorizontalAlignment = Syncfusion.Windows.Forms.Grid.GridHorizontalAlignment.Right

          m_grid(currentCustomerIndex, 5).CellValue = crh.GetValue(Of String)("Status")
          ...
          m_grid(currentDocIndex, 3).CellValue = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAR.Total}") '"รวมเงิน"
          m_grid(currentDocIndex, 4).CellValue = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAR.OpeningBalance}") '"ยอดยกมา"
          m_grid(currentDocIndex, 8).CellValue = Configuration.FormatToString(crh.GetValue(Of Decimal)("OpenningBalance"), DigitConfig.Price)
          m_grid(currentDocIndex, 12).CellValue = Configuration.FormatToString(crh.GetValue(Of Decimal)("RetentionOpenningBalance"), DigitConfig.Price)
          SumOpnBalance += crh.GetValue(Of Decimal)("OpenningBalance")
          SumRetOpnBalance += crh.GetValue(Of Decimal)("RetentionOpenningBalance")
        End If
      Next
