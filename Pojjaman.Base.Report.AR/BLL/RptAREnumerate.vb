...
#Region "Methods"
		...
		Private Sub CreateHeader()
			...
			m_grid(0, 1).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAREnumerate.PrID}")				 '"รหัสลูกหนี้"
			m_grid(0, 2).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAREnumerate.PrName}")			 '"ชื่อลูกหนี้"
			m_grid(0, 3).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAREnumerate.CreditAmount}")		'"วงเงินเครดิต"
			m_grid(0, 4).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAREnumerate.RemainingCreditAmount}")		'"วงเงินเครดิตคงเหลือ"
			m_grid(0, 5).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAREnumerate.PrStatus}")		 '"สถานะลูกหนี้"
			...
			m_grid(1, 1).Text = indent & Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAREnumerate.DocDate}")		 '"วันที่เอกสาร"
			m_grid(1, 2).Text = indent & Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAREnumerate.DocCode}")		 '"เลขที่เอกสาร"
			m_grid(1, 3).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAREnumerate.InVoid}")				'"เลขที่ใบกำกับ"
			m_grid(1, 4).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAREnumerate.DocType}")			 '"ประเภทเอกสาร"
			m_grid(1, 5).Text = indent & Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAREnumerate.PrCode}")			 '"เลขที่ใบสำคัญจ่าย"
			m_grid(1, 6).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAREnumerate.Sold}")				'"ขายเชื่อ"
			m_grid(1, 7).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAREnumerate.SaleSCN}")				'"ลดหนี้"
			m_grid(1, 8).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAREnumerate.Receive}")			 '"รับชำระหนี้"
			m_grid(1, 9).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAREnumerate.Status}")			 '"คงเหลือ"
			m_grid(1, 10).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAREnumerate.Term}")				 '"เทอม"
			m_grid(1, 11).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAREnumerate.MileStoneRetention}")				'"ถูกหัก"
			m_grid(1, 12).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAREnumerate.ReceiveRetention}")				'"รับคืน"
			m_grid(1, 13).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAREnumerate.RetentionBalance}")				'"คงเหลือ"
			m_grid(1, 14).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAREnumerate.Cash}")				 '"รับเงินสด"
			m_grid(1, 15).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAREnumerate.Check}")				 '"เช็ครับ"
			m_grid(1, 16).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAREnumerate.AdvancePay}")				 '"หักเงินมัดจำ"
			m_grid(1, 17).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAREnumerate.Interest}")				 '"ดอกเบี้ยรับ"
			m_grid(1, 18).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAREnumerate.AmtIncrease}")				 '"ยอดรับเพิ่ม"
			m_grid(1, 19).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAREnumerate.Discount}")				 '"ส่วนลด"
			m_grid(1, 20).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAREnumerate.WHT}")				 '"ภาษี ณ ที่จ่าย"
			m_grid(1, 21).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAREnumerate.AmtDeduct}")					'"ยอดหักรับ"
			m_grid(1, 22).Text = indent & Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAREnumerate.Note}")				 '"หมายเหตุ"

			m_grid(2, 3).Text = indent & Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAREnumerate.RefDocCode}")				 '"เอกสารต้นทาง"
			m_grid(2, 4).Text = indent & Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAREnumerate.RefDocDate}")				'"วันที่เอกสารต้นทาง"
			m_grid(2, 5).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAREnumerate.RefDocPayAmt}")				 '"ยอดเงินที่นำมาวางบิล"
			m_grid(2, 15).Text = indent & Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAREnumerate.CheckCode}")				 '"เลขที่เช็ค"
			m_grid(2, 16).Text = indent & Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAREnumerate.CheckDueDate}")				 '"วันที่บนเช็ค"
			m_grid(2, 17).Text = indent & Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAREnumerate.CheckBankAcct}")				 '"บัญชีธนาคาร"
			m_grid(2, 18).Text = indent & Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAREnumerate.CheckStatus}")				 '"สถานะเช็ค"
      ...
      Next
			...
			If dt2.Rows.Count > 0 Then
				...
				m_grid(currentDocIndex, 6).CellValue = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAREnumerate.Sold}")				 '"ขายเชื่อ"
				m_grid(currentDocIndex, 7).CellValue = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAREnumerate.SaleSCN}")				 '"ลดหนี้"
				m_grid(currentDocIndex, 8).CellValue = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAREnumerate.Receive}")				'"รับชำระหนี้"
				m_grid(currentDocIndex, 9).CellValue = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAR.Status}")				 '"คงเหลือ"
				m_grid(currentDocIndex, 11).CellValue = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAREnumerate.MileStoneRetention}")				 '"ถูกหัก"
				m_grid(currentDocIndex, 12).CellValue = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAREnumerate.ReceiveRetention}")				 '"รับคืน"
				m_grid(currentDocIndex, 13).CellValue = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAR.Status}")				 '"คงเหลือ"
				m_grid(currentDocIndex, 3).HorizontalAlignment = Syncfusion.Windows.Forms.Grid.GridHorizontalAlignment.Right
				m_grid(currentDocIndex, 5).HorizontalAlignment = Syncfusion.Windows.Forms.Grid.GridHorizontalAlignment.Right
