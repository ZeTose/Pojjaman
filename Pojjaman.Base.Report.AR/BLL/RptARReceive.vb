...
#Region "Methods"
    ...
    Private Sub CreateHeader()
      ...
      tr("col0") = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptARReceive.ReciveDate}")    '"วันที่ชำระ" 0
      tr("col1") = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptARReceive.DocCode}")     '"เลขที่เอกสารชำระ" 1
      tr("col2") = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptARReceive.ARCode}")   '"รหัสลูกหนี้" +++++++++++
      tr("col3") = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptARReceive.ARName}")   '"ชื่อลูกหนี้" 2
      tr("col4") = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptARReceive.RefDocNO}")  '"เอกสารอ้างอิง" 3
      tr("col5") = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptARReceive.RefDocDate}")  '"วันที่เอกสารอ้างอิง" 4
      tr("col6") = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptARReceive.CCCode}")   '"รหัส Cost Center" ++++++++++++
      tr("col7") = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptARReceive.CCName}")   '"ชื่อ Cost Center" ++++++++++++
      tr("col8") = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptARReceive.StockAmount}")   '"ยอดตามเอกสารอ้างอิง" 5
      tr("col9") = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptARReceive.Gross}")   '"ยอดที่ต้องชำระ" 6
      tr("col10") = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptARReceive.WitholdingTax}")   '"ภาษีถูกหัก ณ ที่รับ" 7
      tr("col11") = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptARReceive.Decrease}")    '"ยอดหักรับ" 8
      tr("col12") = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptARReceive.Increase}")   '"ยอดเพิ่มรับ" 9
      tr("col13") = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptARReceive.Amount}")   '"ยอดรับชำระ" 10
      tr("col14") = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptARReceive.GlNote}")   '"หมายเหตุ" 11
      If ShowDetail <> 0 Then
        ' Level 2.
        ...
        tr("col3") = indent & indent & Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptARReceive.DocType}")    '"เลขที่/ประเภทเอกสาร"
        tr("col4") = indent & indent & Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptARReceive.BankAccount}")    '"เลขที่สมุดบัญชี"
        tr("col5") = indent & indent & Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptARReceive.DocDate}")   '"วันที่ครบกำหนด/วันที่เอกสาร"

        'tr("col6") = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptARReceive.DocGross}")    '"ยอดเงิน"
        tr("col13") = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptARReceive.ReceiveAmount}")    '"รับชำระ"
    ...
    Private Sub PopulateData()
      ...
      trReceive = Me.m_treemanager.Treetable.Childs.Add
      trReceive.Tag = "Font.Bold"
      trReceive("col5") = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptARReceive.Total}") '"รวม"
...
#Region "IPrintableEntity"
    ...
    Public Overrides Function GetDocPrintingEntries() As DocPrintingItemCollection
      ...
      receiveType = Replace(receiveType, "0", "เงินสด")
      Trace.WriteLine(receiveType)
      receiveType = Replace(receiveType, "27", "เช็ค")
      Trace.WriteLine(receiveType)
      receiveType = Replace(receiveType, "72", "โอน")
      Trace.WriteLine(receiveType)
      'ประเภทการรับ
      dpi = New DocPrintingItem
