...
#Region "Methods"
    ...
    Private Sub CreateHeader()
      ...
      m_grid(0, 1).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptMatCount.CostCenterID}") '"รหัส Cost Center"
      m_grid(0, 2).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptMatCount.CostCenterName}") '"ชื่อ Cost Center"

      m_grid(1, 1).Text = indent & Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptMatCount.StockID}") '"รหัสวัสดุ"
      m_grid(1, 2).Text = indent & Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptMatCount.StockName}")  '"ชื่อวัสดุ"
      m_grid(1, 3).Text = indent & Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptMatCount.UnitName}")  '"ชื่อหน่วยนับ"
      m_grid(1, 4).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptMatCount.Num}")  '"จำนวน"
      m_grid(1, 5).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptMatCount.AvgPrice}")  '"ราคาเฉลี่ย"
      m_grid(1, 6).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptMatCount.Capital}")  '"ต้นทุน"
      ...
      For Each row As DataRow In dt.Rows
        ...
        End If
                ...
                If IsNumeric(row("unitprice")) Then
                    m_grid(currItemIndex, 5).CellValue = Configuration.FormatToString(CDec(row("unitprice")) / CDec(row("stockcount")), DigitConfig.Price)
                End If
                currentItemCode = row("LciCode").ToString
                If (CDec(row("Qty")) <> 0 AndAlso CDec(row("unitprice")) <> 0) Then
                    m_grid(currItemIndex, 6).CellValue = Configuration.FormatToString(CDec(row("Qty")) * (CDec(row("unitprice")) / CDec(row("stockcount"))), DigitConfig.UnitPrice)
                Else
...
#End Region#Region "Properties"    Public Overrides ReadOnly Property ClassName() As String
    ...
    Public Overrides ReadOnly Property DetailPanelTitle() As String
      Get
        Return "${res:Longkong.Pojjaman.BusinessLogic.RptMatCount.DetailLabel}"
      End Get
    End Property
    Public Overrides ReadOnly Property DetailPanelIcon() As String
      Get
        Return "Icons.16x16.RptMatCount"
      End Get
    End Property
    ...
    Public Overrides ReadOnly Property ListPanelTitle() As String
      Get
        Return "${res:Longkong.Pojjaman.BusinessLogic.RptMatCount.ListLabel}"
      End Get
    End Property
