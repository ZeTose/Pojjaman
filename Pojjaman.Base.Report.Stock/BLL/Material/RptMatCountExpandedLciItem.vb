...
#Region "Methods"
    ...
    '  m_grid(1, 2).Text = indent & Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.CRptMatCountExpandedLciItem.StockName}")  '"ชื่อวัสดุ"
    '  m_grid(1, 3).Text = indent & Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.CRptMatCountExpandedLciItem.UnitName}")  '"ชื่อหน่วยนับ"
    '  m_grid(1, 4).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.CRptMatCountExpandedLciItem.Num}")  '"จำนวน"
    '  m_grid(1, 5).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.CRptMatCountExpandedLciItem.AvgPrice}")  '"ราคาเฉลี่ย"
    '  m_grid(1, 6).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.CRptMatCountExpandedLciItem.Capital}")  '"ต้นทุน"
    ...
    '        m_grid(currItemIndex, 5).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.CRptMatCountExpandedLciItem.ZeroStock}")
    '      Else
    '        m_grid(currItemIndex, 5).CellValue = Configuration.FormatToString(CDec(row("Costamt")) / CDec(row("QTY")), DigitConfig.Price)
    '      End If
    '    Else
    '      m_grid(currItemIndex, 5).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.CRptMatCountExpandedLciItem.NoCost}")
    '    End If
    ...
    '    Else
    '      m_grid(currItemIndex, 6).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.CRptMatCountExpandedLciItem.NoCost}")
    '    End If
