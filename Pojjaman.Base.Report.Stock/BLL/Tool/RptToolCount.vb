...
#Region "Methods"
        ...
        Private Sub CreateHeader()
            ...
            m_grid(0, 1).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptToolCount.CostCenterID}") '"รหัส Cost Center"
            m_grid(0, 2).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptToolCount.CostCenterName}") '"ชื่อ Cost Center"

            m_grid(1, 1).Text = indent & Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptToolCount.ToolGroupID}") '"รหัสกลุ่มเครื่องมือ"
            m_grid(1, 2).Text = indent & Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptToolCount.ToolGroupName}")  '"ชื่อกลุ่มเครื่องมือ"
            m_grid(1, 3).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptToolCount.ToolID}")  '"รหัสวัสดุ"
            m_grid(1, 4).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptToolCount.ToolName}")  '"ชื่อวัสดุ"
            m_grid(1, 5).Text = indent & Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptToolCount.UnitName}")  '"ชื่อหน่วยนับ"
            m_grid(1, 6).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptToolCount.Num}")  '"จำนวน"
        ...
        Private Sub PopulateData()
            ...
            m_grid.RowStyles(currItemIndex).Font.Bold = True
            m_grid.RowStyles(currItemIndex).ReadOnly = True
            m_grid(currItemIndex, 5).CellValue = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptMatStockMonitor.SumAmount}")  '"รวมทั้งหมด"
