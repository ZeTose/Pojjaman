...
#Region "Methods"
    ...
    Private Sub CreateHeader()
      ...
      m_grid(0, 5).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptSC.gross}")             '"ยอดเงินรวม"
      m_grid(0, 6).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptSC.discamt}")             '"ส่วนลด"
      m_grid(0, 7).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptSC.afterdiscount}")       '"ยอดเงินหลังหักส่วนลด"

      m_grid(0, 8).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptSC.Sum}")             '"ยอดรวม"
      m_grid(0, 9).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptSC.tax}")             '"ภาษี"
      m_grid(0, 10).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptSC.beforetax}")       '"ยอดรวมภาษี"

      m_grid(0, 11).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptSC.note}")      '"หมายเหตุ"
      m_grid(0, 12).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptSC.closestatus}")    '"สถานะการเปิด PO"
      m_grid(0, 13).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptSC.refstatus}")      '"อ้างอิง"
      m_grid(0, 14).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptSC.scstatusinfo}")    '"สถานะ"
      m_grid(0, 15).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptSC.Director}")      '"ผู้สั่งจ้าง"
    ...
    Private Sub PopulateData()
      ...
      Dim SumAfterDisCount As Decimal = 0
      ...
      For Each row As DataRow In dt.Rows
        ...
        If Not row.IsNull("sc_gross") Then
          m_grid(currItemIndex, 5).CellValue = Configuration.FormatToString(CDec(row("sc_gross")), DigitConfig.Price)
          SumGross += CDec(row("sc_gross"))
        End If
        If Not row.IsNull("sc_discamt") Then
          m_grid(currItemIndex, 6).CellValue = Configuration.FormatToString(CDec(row("sc_discamt")), DigitConfig.Price)
          SumDisCount += CDec(row("sc_discamt"))
        End If
        If Not row.IsNull("sc_gross") Then
          m_grid(currItemIndex, 7).CellValue = Configuration.FormatToString(CDec(row("sc_gross")) - CDec(row("sc_discamt")), DigitConfig.Price)
          SumAfterDisCount += (CDec(row("sc_gross")) - CDec(row("sc_discamt")))
        End If

        If Not row.IsNull("sc_beforetax") Then
          m_grid(currItemIndex, 8).CellValue = Configuration.FormatToString(CDec(row("sc_beforetax")), DigitConfig.Price)
          SumAmount += CDec(row("sc_beforetax"))
        End If
        If Not row.IsNull("sc_taxamt") Then
          m_grid(currItemIndex, 9).CellValue = Configuration.FormatToString(CDec(row("sc_taxamt")), DigitConfig.Price)
          SumTaxAmt += CDec(row("sc_taxamt"))
        End If
        If Not row.IsNull("sc_aftertax") Then
          m_grid(currItemIndex, 10).CellValue = Configuration.FormatToString(CDec(row("sc_aftertax")), DigitConfig.Price)
          SumAfterTax += CDec(row("sc_aftertax"))
        End If
        m_grid(currItemIndex, 11).CellValue = row("SCnote").ToString
        m_grid(currItemIndex, 12).CellValue = row("closestatus").ToString
        m_grid(currItemIndex, 13).CellValue = row("refstatus").ToString

        m_grid(currItemIndex, 14).CellValue = row("SCstatusinfo").ToString

        m_grid(currItemIndex, 15).CellValue = row("Director").ToString
      ...
      m_grid(currTrIndex, 5).CellValue = Configuration.FormatToString(SumGross, DigitConfig.Price) 'ยอดเงินรวม
      m_grid(currTrIndex, 6).CellValue = Configuration.FormatToString(SumDisCount, DigitConfig.Price) 'ส่วนลด
      m_grid(currTrIndex, 7).CellValue = Configuration.FormatToString(SumAfterDisCount, DigitConfig.Price) 'ยอดหลังส่วนลด

      m_grid(currTrIndex, 8).CellValue = Configuration.FormatToString(SumAmount, DigitConfig.Price) 'ยอดเงิน
      m_grid(currTrIndex, 9).CellValue = Configuration.FormatToString(SumTaxAmt, DigitConfig.Price) 'ยอดภาษี
      m_grid(currTrIndex, 10).CellValue = Configuration.FormatToString(SumAfterTax, DigitConfig.Price) 'ยอดรวมภาษี 
