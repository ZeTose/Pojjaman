...
#Region "Methods"
        ...
        Private Sub CellDblClick(ByVal sender As Object, ByVal e As Syncfusion.Windows.Forms.Grid.GridCellClickEventArgs)
            ...
            Dim docId As Integer = drh.GetValue(Of Integer)("vo_id")
            Dim docType As Integer = 290
        ...
        Private Sub CreateHeader()
            ...
            m_grid(0, 1).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptVO.DocNumber}")       '"เลขที่เอกสาร"
            m_grid(0, 2).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptVO.vo_docDate}")      '"วันที่เอกสาร"
            m_grid(0, 3).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptVO.SCCode}")
            m_grid(0, 4).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptVO.subcontractor}")    '"ผู้รับเหมา"    
            m_grid(0, 5).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptVO.ccinfo}")          '"Cost Center "
            m_grid(0, 6).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptVO.Sum}")             '"ยอดรวม"
            m_grid(0, 7).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptVO.beforetax}")       '"ยอดรวมภาษี"
            m_grid(0, 8).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptVO.note}")      '"หมายเหตุ"
            m_grid(0, 9).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptVO.refstatus}")      '"อ้างอิง"
            m_grid(0, 10).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptVO.scstatusinfo}")    '"สถานะ"
            m_grid(0, 11).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptVO.Director}")      '"ผู้สั่งจ้าง"
        ...
        Private Sub PopulateData()
            ...
            m_grid.RowStyles(currItemIndex).ReadOnly = True
            m_grid(currTrIndex, 5).CellValue = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptVO.SumAll}") 'รวม
