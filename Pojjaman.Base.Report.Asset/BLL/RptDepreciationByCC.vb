...
Namespace Longkong.Pojjaman.BusinessLogic
    Public Class RptDepreciationByCC
        Inherits Report
...
#Region "Methods"
        ...
        Private Sub CreateHeader()
            ...
            m_grid(0, 1).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptDepreciationByCC.CostCenter}") '"รหัส Cost center"
            m_grid(0, 2).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptDepreciationByCC.CCName}") '"ชื่อ Cost center"

            m_grid(1, 2).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptDepreciationByCC.AssetCode}") '"สินทรัพย์"
            m_grid(1, 3).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptDepreciationByCC.Price}") '"ราคาซื้อ"
            m_grid(1, 4).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptDepreciationByCC.StartDate}") '"เริ่มต้น"
            m_grid(1, 5).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptDepreciationByCC.EndDate}") '"สิ้นสุด"
            m_grid(1, 6).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptDepreciationByCC.Depreciation}") '"ค่าเสื่อม"
            m_grid(1, 7).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptDepreciationByCC.SumDepreciation}") '"ค่าเสื่อมสะสม"
            m_grid(1, 8).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptDepreciationByCC.NetAssetValue}") '"มูลค่าคงเหลือ"
            m_grid(1, 9).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptDepreciationByCC.DocRefer}") '"เอกสารอ้างอิง"
        ...
        Private Sub PopulateData()
            ...
            For Each row As DataRow In dt.Rows
                If row("CCcode").ToString <> currentCCCode Then
                    If currentCCCode.Length > 0 Then
                        m_grid.RowCount += 1
                        currCCIndex = m_grid.RowCount
                        m_grid.RowStyles(currCCIndex).BackColor = Color.FromArgb(167, 214, 231)
                        m_grid(currCCIndex, 2).CellValue = "รวม"
                        m_grid(currCCIndex, 3).CellValue = Configuration.FormatToString(sumPrice, DigitConfig.Price)
                        m_grid(currCCIndex, 6).CellValue = Configuration.FormatToString(sumDeprec, DigitConfig.Price)
                        m_grid(currCCIndex, 7).CellValue = Configuration.FormatToString(sumDeprecNet, DigitConfig.Price)
                        m_grid(currCCIndex, 8).CellValue = Configuration.FormatToString(sumNetAssetValue, DigitConfig.Price)
                    End If
                    m_grid.RowCount += 1
                    currCCIndex = m_grid.RowCount
                ...
                If Not row.IsNull("AsstName") Then
                    m_grid(currIndex, 2).CellValue = row("AsstName").ToString
                End If
            ...
            m_grid.RowStyles(currCCIndex).BackColor = Color.FromArgb(167, 214, 231)
            m_grid(currCCIndex, 2).CellValue = "รวม"
            m_grid(currCCIndex, 2).HorizontalAlignment = Syncfusion.Windows.Forms.Grid.GridHorizontalAlignment.Left
            m_grid(currCCIndex, 3).CellValue = Configuration.FormatToString(sumPrice, DigitConfig.Price)
            m_grid(currCCIndex, 6).CellValue = Configuration.FormatToString(sumDeprec, DigitConfig.Price)
            m_grid(currCCIndex, 7).CellValue = Configuration.FormatToString(sumDeprecNet, DigitConfig.Price)
            m_grid(currCCIndex, 8).CellValue = Configuration.FormatToString(sumNetAssetValue, DigitConfig.Price)
            ...
            m_grid.RowStyles(currCCIndex).Font.Bold = True
            m_grid.RowStyles(currCCIndex).ReadOnly = True
            m_grid(currCCIndex, 2).CellValue = "รวมทั้งหมด"
            m_grid(currCCIndex, 2).HorizontalAlignment = Syncfusion.Windows.Forms.Grid.GridHorizontalAlignment.Left
            m_grid(currCCIndex, 3).CellValue = Configuration.FormatToString(TotalPrice, DigitConfig.Price)
            m_grid(currCCIndex, 6).CellValue = Configuration.FormatToString(TotalDeprec, DigitConfig.Price)
            m_grid(currCCIndex, 7).CellValue = Configuration.FormatToString(TotalDeprecNet, DigitConfig.Price)
            m_grid(currCCIndex, 8).CellValue = Configuration.FormatToString(TotalNetAssetValue, DigitConfig.Price)

        End Sub
#End Region#Region "Shared"
#End Region#Region "Properties"        Public Overrides ReadOnly Property ClassName() As String
            Get
                Return "RptDepreciationByCC"
            End Get
        End Property
        Public Overrides ReadOnly Property DetailPanelTitle() As String
            Get
                Return "${res:Longkong.Pojjaman.BusinessLogic.RptDepreciationByCC.DetailLabel}"
            End Get
        End Property
        Public Overrides ReadOnly Property DetailPanelIcon() As String
            Get
                Return "Icons.16x16.RptDepreciationByCC"
            End Get
        End Property
        ...
        Public Overrides ReadOnly Property ListPanelTitle() As String
            Get
                Return "${res:Longkong.Pojjaman.BusinessLogic.RptDepreciationByCC.ListLabel}"
            End Get
        End Property
