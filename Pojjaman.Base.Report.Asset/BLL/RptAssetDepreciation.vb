...
#Region "Methods"
        ...
        Private Sub CreateHeader()
            ...
            m_grid(1, 6).Text = indent & Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAssetDepreciation.cc_code}")   '"รหัส costcenter"
            m_grid(1, 7).Text = indent & Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAssetDepreciation.cc_name}")   '"ชื่อ costcenter"
            m_grid(1, 8).Text = indent & Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAssetDepreciation.BuyPrice}")  '"ราคา"
            m_grid(1, 9).Text = indent & Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAssetDepreciation.Salvage}")  '"ราคาค่าซาก"
            m_grid(1, 10).Text = indent & Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAssetDepreciation.Openning}")   '"ยอดยกมา"
            m_grid(1, 11).Text = indent & Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAssetDepreciation.CalcRate}")   '"อัตรา (%)"
            m_grid(1, 12).Text = indent & Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAssetDepreciation.DaysOfDepre}")   '"จำนวนวันที่คิดค่าเสื่อม"
            m_grid(1, 13).Text = indent & Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAssetDepreciation.Depreciation}")    '"ค่าสึกหรอ"
            m_grid(1, 14).Text = indent & Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAssetDepreciation.DepreciationOpenning}")   '"ค่าเสื่อมราคาสะสม"
            m_grid(1, 15).Text = indent & Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAssetDepreciation.OnGoing}")   '"ยอดยกไป"
        ...
        Private Sub PopulateData()
            ...
            For Each row As DataRow In dt.Rows
                ...
                If currAccountCode <> darow.GetValue(Of String)("AcctCode") Then
                    If no > 1 Then
                        ...
                        m_grid(currAccountIndex, 10).CellValue = Configuration.FormatToString(sumOpeningBalance, DigitConfig.Price)
                        m_grid(currAccountIndex, 13).CellValue = Configuration.FormatToString(sumDepre, DigitConfig.Price)
                        m_grid(currAccountIndex, 14).CellValue = Configuration.FormatToString(sumDepreAmount, DigitConfig.Price)
                        m_grid(currAccountIndex, 15).CellValue = Configuration.FormatToString(sumEndingBalance, DigitConfig.Price)
                        sumPrice = 0
                        sumOpeningBalance = 0
                        sumDepre = 0
                        sumDepreAmount = 0
                ...
                m_grid(currAssetIndex, 10).CellValue = Configuration.FormatToString(darow.GetValue(Of Decimal)("OpeningBalance"), DigitConfig.Price)
                m_grid(currAssetIndex, 11).CellValue = indent & Configuration.FormatToString(darow.GetValue(Of Decimal)("asset_calcRate"), DigitConfig.Price)
                m_grid(currAssetIndex, 12).CellValue = indent & Configuration.FormatToString(darow.GetValue(Of Decimal)("SumDayOfDepre"), DigitConfig.Price)
                m_grid(currAssetIndex, 13).CellValue = Configuration.FormatToString(darow.GetValue(Of Decimal)("Depre"), DigitConfig.Price)
                m_grid(currAssetIndex, 14).CellValue = Configuration.FormatToString(darow.GetValue(Of Decimal)("DepreAmount"), DigitConfig.Price)
                m_grid(currAssetIndex, 15).CellValue = Configuration.FormatToString(darow.GetValue(Of Decimal)("EndingBalance"), DigitConfig.Price)
            ...
            m_grid(currAssetIndex, 10).CellValue = Configuration.FormatToString(sumAllOpeningBalance, DigitConfig.Price)
            m_grid(currAssetIndex, 13).CellValue = Configuration.FormatToString(sumAllDepre, DigitConfig.Price)
            m_grid(currAssetIndex, 14).CellValue = Configuration.FormatToString(sumAllDepreAmount, DigitConfig.Price)
            m_grid(currAssetIndex, 15).CellValue = Configuration.FormatToString(sumAllEndingBalance, DigitConfig.Price) 'sumEdbNetAmt, DigitConfig.Price)
