...
Namespace Longkong.Pojjaman.Gui.Panels
  Public Class BillAcceptanceSelectionView
...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
      ...
      '
      'BillAcceptanceSelectionView
      '
      Me.Controls.Add(Me.tgItem)
      Me.Controls.Add(Me.Splitter1)
      Me.Controls.Add(Me.pnlFilter)
      Me.Name = "BillAcceptanceSelectionView"
...
#Region "Constructors"
    Public Sub New(ByVal entity As ISimpleEntity, ByVal handler As Object, ByVal basket As BasketDialog, ByVal filters As Filter(), ByVal entities As ArrayList)
      ...
      'Hack
      m_filterSubPanel = New BillAcceptanceFilterSubPanel
      ...
      If TypeOf handler Is NamedEntityOperationDelegate Then
        m_selectionMode = Selection.SingleSelect
      End If
...
#Region "Methods"
    ...
    Public Sub SearchData(ByVal order As String)
      ...
      If otherLength > 0 Then
        ...
        Next
      End If
      Dim dt As TreeTable = BillAcceptanceItem.GetSelectionListDatatable(newfilters)
        ...
        For Each row As TreeRow In myTable.Childs
          For Each childRow As TreeRow In row.Childs
            If Not IsDBNull(childRow("Selected")) Then
              If CBool(childRow("Selected")) Then
                Dim id As Integer = CInt(childRow("billa_id"))
                Dim stockCode As String = CStr(childRow("Code"))
                Dim fullClassName As String = "Longkong.Pojjaman.BusinessLogic.BillAcceptanceItem"
                Dim entityName As String = CStr(childRow("Entity"))
                Dim lineNumber As Integer = CInt(childRow("LineNumber"))
                Dim textInBasket As String = entityName
                Dim bi As New StockBasketItem(id, stockCode, fullClassName, textInBasket, lineNumber, 0, entityName)
                Dim ba As New BillAcceptance
                ba.Id = bi.Id
                ba.Code = bi.StockCode
                Dim newItem As New BillAcceptanceItem
                newItem.BillAcceptance = ba
                If Not childRow.IsNull("stock_id") Then
                  newItem.Id = CInt(childRow("stock_id"))
                End If
                newItem.Code = bi.EntityName
                newItem.ParentCode = ba.Code
                newItem.ParentId = ba.Id
                newItem.ParentType = ba.EntityId
                If Not childRow.IsNull("Date") Then
                  newItem.Date = CDate(childRow("Date"))
                End If
                If Not childRow.IsNull("stock_aftertax") Then
                  newItem.AfterTax = CDec(childRow("stock_aftertax"))
                End If
                If Not childRow.IsNull("stock_beforetax") Then
                  newItem.BeforeTax = CDec(childRow("stock_beforetax"))
                End If
                If Not childRow.IsNull("stock_taxbase") Then
                  newItem.TaxBase = CDec(childRow("stock_taxbase"))
                End If
                If Not childRow.IsNull("billai_unpaidamt") Then
                  newItem.UnpaidAmount = CDec(childRow("billai_unpaidamt"))  'receivable.GetRemainingAmountReceiveSelection(receives_id)
                End If
                If Not childRow.IsNull("billai_billedamt") Then
                  newItem.BilledAmount = CDec(childRow("billai_billedamt"))
                End If
                If Not childRow.IsNull("stock_type") Then
                  newItem.SetType(CInt(childRow("stock_type")))
                End If
                If Not childRow.IsNull("billai_linenumber") Then
                  newItem.Linenumber = CInt(childRow("billai_linenumber"))
                End If
                If Not childRow.IsNull("DueDate") Then
                  Dim due As Date = CDate(childRow("DueDate"))
                  newItem.CreditPeriod = CInt(DateDiff(DateInterval.Day, newItem.Date, due))
                End If
                If Not childRow.IsNull("billai_retentiontype") Then
