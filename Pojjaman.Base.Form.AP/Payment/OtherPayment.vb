...
#Region " Windows Form Designer generated code "
        ...
        <System.Diagnostics.DebuggerStepThrough()> Protected Sub InitializeComponent()
            ...
            Dim resources As System.Resources.ResourceManager = New System.Resources.ResourceManager(GetType(OtherPayment))
...
#Region "Members"
        ...
        Private m_collection As PaymentAccountItemCollection
        Private m_oldCollection As PaymentAccountItemCollection
        Private m_isDebit As Boolean
#End Region

#Region "Constructors"
        Public Sub New(ByVal myPayment As Payment, ByVal isDebit As Boolean)
            MyBase.New()
            Me.InitializeComponent()
            Me.SetLabelText()
            Initialize()

            Dim dt As TreeTable = Payment.GetDebitCreditSchemaTable
            ...
            m_payment = myPayment
            If m_isDebit Then
                m_collection = Me.m_payment.DebitCollection
            Else
                m_collection = Me.m_payment.CreditCollection
            End If

            m_oldCollection = CType(Me.m_collection.Clone, PaymentAccountItemCollection)
...
#Region "Style"
        Public Function CreateTableStyle() As DataGridTableStyle
            ...
            dst.MappingName = "OtherPayment"
            Dim myStringParserService As StringParserService = CType(ServiceManager.Services.GetService(GetType(StringParserService)), StringParserService)

            Dim csLineNumber As New TreeTextColumn
            csLineNumber.MappingName = "Linenumber"
            csLineNumber.HeaderText = myStringParserService.Parse("${res:Longkong.Pojjaman.Gui.Panels.OtherPayment.LineNumberHeaderText}")
            ...
            csCode.HeaderText = myStringParserService.Parse("${res:Longkong.Pojjaman.Gui.Panels.OtherPayment.CodeHeaderText}")
            csCode.NullText = ""
            ...
            csName.HeaderText = myStringParserService.Parse("${res:Longkong.Pojjaman.Gui.Panels.OtherPayment.DescriptionHeaderText}")
            csName.NullText = ""
            csName.Width = 180
            ...
            csAmount.MappingName = "paymenta_amt"
            csAmount.HeaderText = myStringParserService.Parse("${res:Longkong.Pojjaman.Gui.Panels.OtherPayment.AmountHeaderText}")
            csAmount.NullText = ""
            csAmount.DataAlignment = HorizontalAlignment.Right
            csAmount.Format = "#,###.##"
            csAmount.TextBox.Name = "paymenta_amt"
            csAmount.Width = 60
...
#Region "Properties"
        Private ReadOnly Property CurrentItem() As PaymentAccountItem
            Get
                Dim row As TreeRow = Me.m_treeManager.SelectedRow
                If row Is Nothing Then
                    Return Nothing
                End If
                If Not TypeOf row.Tag Is PaymentAccountItem Then
                    Return Nothing
                End If
                Return CType(row.Tag, PaymentAccountItem)
            End Get
        End Property
#End Region
...
#Region "TreeTable Handlers"
        ...
        Private Sub Treetable_ColumnChanging(ByVal sender As Object, ByVal e As System.Data.DataColumnChangeEventArgs)
            ...
            If Me.CurrentItem Is Nothing Then
                Dim doc As New PaymentAccountItem
        ...
        Public Sub ValidateRow(ByVal e As DataColumnChangeEventArgs)
            ...
            Dim paymenta_amt As Object = e.Row("paymenta_amt")

            Select Case e.Column.ColumnName.ToLower
                Case "code"
                    code = e.ProposedValue
                Case "paymenta_amt"
                    paymenta_amt = e.ProposedValue
                Case Else
                    Return
            End Select
            ...
            End If
            If IsDBNull(paymenta_amt) OrElse Not IsNumeric(paymenta_amt) OrElse CDec(paymenta_amt) <= 0 Then
                e.Row.SetColumnError("paymenta_amt", Me.StringParserService.Parse("${res:Global.Error.PaymentAccountItemMissing}"))
            Else
                e.Row.SetColumnError("paymenta_amt", "")
            End If
        End Sub
        ...
        Public Sub SetAmount(ByVal e As DataColumnChangeEventArgs)
            If m_updating Then
                Return
            End If
            Dim doc As PaymentAccountItem = Me.CurrentItem
        ...
        Private Function DupCode(ByVal e As DataColumnChangeEventArgs) As Boolean
            ...
            If doc Is Nothing Then
                Return False
            End If
            For Each item As PaymentAccountItem In Me.m_collection
        ...
        Public Sub SetCode(ByVal e As DataColumnChangeEventArgs)
            ...
            Dim msgServ As IMessageService = CType(ServiceManager.Services.GetService(GetType(IMessageService)), IMessageService)
            If DupCode(e) Then
                msgServ.ShowMessageFormatted("${res:Global.Error.AlreadyHasCode}", New String() {"ผังบัญชี", e.ProposedValue.ToString})
                e.ProposedValue = e.Row(e.Column)
                m_updating = False
                Return
            End If
...
#Region "IListDetail"
        Public Overrides Sub CheckFormEnable()
            If Me.m_payment Is Nothing Then
                Return
            End If
            If Me.m_payment.Status.Value >= 3 OrElse Me.m_payment.Status.Value = 0 Then
        ...
        Public Overrides Sub SetLabelText()
            If Not m_payment Is Nothing Then Me.Text = Me.StringParserService.Parse(Me.m_payment.TabPageText)
            If Me.m_isDebit Then
                Me.grbDetail.Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.Gui.Panels.PaymentOutDetail.lblDebitAmount}")
            Else
                Me.grbDetail.Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.Gui.Panels.PaymentOutDetail.lblCreditAmount}")
            End If
            Me.lblGross.Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.Gui.Panels.OtherPayment.lblGross}")
...
#Region "Event Handlers"
        ...
        Private Sub SetAccts(ByVal items As BasketItemCollection)
            ...
            For i As Integer = items.Count - 1 To 0 Step -1
                ...
                Dim myItem As New PaymentAccountItem
        ...
        Private Sub ibtnBlank_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ibtnBlank1.Click
            ...
            If index = -1 OrElse index > Me.m_collection.Count - 1 Then
                Me.m_collection.Add(New PaymentAccountItem)
            Else
                Me.m_collection.Insert(index, New PaymentAccountItem)
            End If
            RefreshDocs()
            tgItem.CurrentRowIndex = index
        End Sub
        Private Sub ibtnDelRow_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ibtnDelRow1.Click
            ...
            If row Is Nothing Then
                Return
            End If
            Dim item As PaymentAccountItem = Me.CurrentItem
...
#Region "Buttons Event"
        ...
        Private Sub btnCancel_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnCancel1.Click
            If m_isDebit Then
                Me.m_payment.DebitCollection = m_oldCollection
            Else
                Me.m_payment.CreditCollection = m_oldCollection
            End If
        End Sub
#End Region
