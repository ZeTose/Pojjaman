...
#Region "Methods"
        ...
        Private Sub CreateHeader()
            ...
            tr("col2") = indent & Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptJournalEntry.RefDocCode}") '"เลขที่เอกสารอ้างอิง"
            ...
            tr("col6") = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptACCBudgetByCC.Budget}") '"Budget"
            tr("col7") = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptACCBudgetByCC.AccBalance}") '"เดบิต"
        ...
        Private Sub PopulateData()
            ...
            For Each row As DataRow In dt.Rows
                ...
                theRow.Tag = rowTag
                Dim budget As Decimal = CDec(IIf(row.IsNull("budget"), 0, row("budget")))
                Dim type As Integer = CInt(IIf(row.IsNull("acct_type"), 0, row("acct_type")))
                Me.SetTransaction(rowTag, theRow, budget, type)
            ...
            For Each row As DataRow In dt.Rows
                ...
                If Not parnode Is Nothing AndAlso parnode.Childs.Count > 0 AndAlso (ShowSumEachAcct OrElse CInt(row("acct_id")) = 0) Then
                    ...
                    budget = SumChilds(budget, parnode, "col6")
                    debit = SumChilds(debit, parnode, "col7")
                    balance = SumChilds(balance, parnode, "col8")
                    theRow("col6") = Configuration.FormatToString(budget, DigitConfig.Price)
                    theRow("col7") = Configuration.FormatToString(debit, DigitConfig.Price)
                    theRow("col8") = Configuration.FormatToString(balance, DigitConfig.Price)
                    therow.Tag = "summary"
                End If
            Next
            Me.m_treemanager.Treetable.AcceptChanges()
        ...
        Private Function SearchTag(ByVal id As Integer) As TreeRow
            ...
            Next
        End Function
        Private Sub SetTransaction(ByVal id As Integer, ByVal tr As TreeRow, ByVal budget As Decimal, ByVal type As Integer)
            If Me.DataSet.Tables.Count > 1 Then
                ...
                For Each row As DataRow In dt.Select("acct_id = " & id.ToString)
                    If Not row.IsNull("gl_id") Then
                        ...
                        Dim gli_balanceamt As Decimal = 0 'CDec(IIf(row.IsNull("gli_balanceamt"), 0, row("gli_balanceamt")))
                        ...
                        Select Case type
                            Case 2, 5
                                gli_debitamt -= gli_creditamt
                                gli_creditamt = 0
                                gli_balanceamt = budget - gli_debitamt
                            Case 1, 4, 3
                                gli_creditamt -= gli_debitamt
                                gli_debitamt = gli_creditamt
                                gli_balanceamt = budget - gli_debitamt
                        ...
                        If Not row.IsNull("gl_docdate") Then
                            theRow("col0") = Space(6) & CDate(row("gl_docdate")).ToString("dd/MM/yyyy")
                        End If
                        If Not row.IsNull("gl_code") Then
                            theRow("col1") = Space(6) & CStr(row("gl_code"))
                        End If
                        If Not row.IsNull("gl_refcode") Then
                            theRow("col2") = Space(6) & CStr(row("gl_refcode"))
                        End If
                        ...
                        theRow("col6") = Configuration.FormatToString(budget, DigitConfig.Price)      ' Budget
                        theRow("col7") = Configuration.FormatToString(gli_debitamt, DigitConfig.Price)     ' เดบิต
...
#End Region#Region "Properties"        Public Overrides ReadOnly Property ClassName() As String
            Get
                Return "RptACCBudgetByCC"
            End Get
        End Property
        Public Overrides ReadOnly Property DetailPanelTitle() As String
            Get
                Return "${res:Longkong.Pojjaman.BusinessLogic.RptACCBudgetByCC.DetailLabel}"
            End Get
        End Property
        Public Overrides ReadOnly Property DetailPanelIcon() As String
            Get
                Return "Icons.16x16.RptACCBudgetByCC"
            End Get
        End Property
        ...
        Public Overrides ReadOnly Property ListPanelTitle() As String
            Get
                Return "${res:Longkong.Pojjaman.BusinessLogic.RptACCBudgetByCC.ListLabel}"
            End Get
        End Property
