...
namespace Longkong.XmlForms {
	...
	public class XmlLoader
	{
		...
		readonly static Regex fontRegex = new Regex(@"Name=(.+),Size=(.+),Style=(.+),Unit=(.+)");
			...
			try {
				...
				if (val.StartsWith("{") && val.EndsWith("}")) { // parse {Property1=value, Property2=value, ... Propertyn=value} style values
					...
					if (m.Success) 
					{
						...
						if(m.Groups[3].Value=="Bold")
						{
							st=FontStyle.Bold;
						}
						else if(m.Groups[3].Value=="Italic")
						{
							st=FontStyle.Italic;
						}
						else if(m.Groups[3].Value=="Bold,Italic")
						{
							st=FontStyle.Bold;
							st |=FontStyle.Italic;
						}
						...
						//System.Windows.Forms.MessageBox.Show(m.Groups[1].Value+","+m.Groups[2].Value+","+m.Groups[3].Value);
						Font f = new Font(m.Groups[1].Value,Convert.ToSingle(m.Groups[2].Value),st,GraphicsUnit.Point,0);
						propertyInfo.SetValue(o,f, null);
					} 
					else 
					{
