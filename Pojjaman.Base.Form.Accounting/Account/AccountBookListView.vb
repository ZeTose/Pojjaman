...
Namespace Longkong.Pojjaman.Gui.Panels
    Public Class AccountBookListView
...
#Region " Windows Form Designer generated code "
        ...
        <System.Diagnostics.DebuggerStepThrough()> Protected Sub InitializeComponent()
            ...
            Dim resources As System.Resources.ResourceManager = New System.Resources.ResourceManager(GetType(AccountBookListView))
            ...
            Me.tgItem.Size = New System.Drawing.Size(656, 312)
            ...
            Me.ibtnBlank.Location = New System.Drawing.Point(128, 40)
            ...
            Me.ibtnDelRow.Location = New System.Drawing.Point(152, 40)
            ...
            Me.lblCode.Text = "รหัส/ชื่อ:"
            Me.lblCode.TextAlign = System.Drawing.ContentAlignment.MiddleRight
            '
            'btnSearch
            '
            Me.btnSearch.FlatStyle = System.Windows.Forms.FlatStyle.System
            Me.btnSearch.Location = New System.Drawing.Point(296, 6)
...
#Region "Members"
        Private m_entity As AccountBook
        ...
        Private m_accountbookCollection As AccountBookCollection
...
#Region "Properties"
        Public Property AccountBookCollection() As AccountBookCollection            Get                Return m_accountbookCollection            End Get            Set(ByVal Value As AccountBookCollection)                m_accountbookCollection = Value            End Set        End Property
#End Region
...
#Region "ISimpleListPanel"
        ...
        Public Sub SetLabelText() Implements ISimplePanel.SetLabelText
            ...
            Me.lblItem.Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.Gui.Panels.AccountBookListView.lblItem}")
            Me.lblCode.Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.Gui.Panels.AccountBookListView.lblCode}")
...
#Region "Style"
        Public Function CreateTableStyle() As DataGridTableStyle
            ...
            dst.GridColumnStyles.Add(csTitleName)
            dst.GridColumnStyles.Add(csPrefix)
            Return dst
        End Function
        Public Function GetSchemaTable() As TreeTable
            Dim myDatatable As New TreeTable("AccountBook")
            ...
            myDatatable.Columns.Add(New DataColumn("TitleName", GetType(String)))
            myDatatable.Columns.Add(New DataColumn("Prefix", GetType(String)))
            Return myDatatable
        End Function
#End Region
...
#Region "TreeTable Handlers"
        ...
        Public Sub ValidateRow(ByVal e As DataColumnChangeEventArgs)
            ...
            If Not isBlankRow Then
                If IsDBNull(code) OrElse CStr(code).Length = 0 Then
                    e.Row.SetColumnError("code", Me.StringParserService.Parse("${res:Global.Error.NoAccountBookCode}"))
                Else
                    e.Row.SetColumnError("code", "")
                End If
                If IsDBNull(name) OrElse CStr(name).Length = 0 Then
                    e.Row.SetColumnError("name", Me.StringParserService.Parse("${res:Global.Error.NoAccountBookName}"))
                Else
        ...
        Private Function DupCodeOrName(ByVal e As DataColumnChangeEventArgs) As Boolean
            ...
            For Each row As TreeRow In Me.m_treeManager.Treetable.Rows
                ...
                End If
            Next
            Dim testAccountBook As New AccountBook(e.ProposedValue.ToString)
            If m_AccountBook Is Nothing OrElse m_AccountBook.Originated Then
                If testAccountBook.Originated Then
                    Return True
                End If
            End If
            If Not m_AccountBook Is Nothing AndAlso m_AccountBook.Originated Then
                If testAccountBook.Originated And testAccountBook.Id <> m_AccountBook.Id Then
                    Return True
                End If
            End If
            Return False
        ...
        Public Sub SetCode(ByVal e As System.Data.DataColumnChangeEventArgs)
            ...
            If DupCodeOrName(e) Then
                msgServ.ShowMessageFormatted("${res:Global.Error.AlreadyHasCodeOrName}", New String() {m_AccountBook.DetailPanelTitle, e.ProposedValue.ToString})
                e.ProposedValue = e.Row(e.Column)
                m_updating = False
                Return
            End If
            m_AccountBook.Code = e.ProposedValue.ToString
            m_updating = False
        End Sub
        Public Sub SetName(ByVal e As System.Data.DataColumnChangeEventArgs)
            ...
            If DupCodeOrName(e) Then
                ...
                Return
            End If
            m_AccountBook.Name = e.ProposedValue.ToString
            m_updating = False
        End Sub
        Public Sub SetTitleName(ByVal e As System.Data.DataColumnChangeEventArgs)
            If m_updating Then
                Return
            End If
            m_updating = True
            ...
            End If
            Dim msgServ As IMessageService = CType(ServiceManager.Services.GetService(GetType(IMessageService)), IMessageService)
            m_AccountBook.TitleName = e.ProposedValue.ToString
            m_updating = False
        End Sub
        Public Sub SetPrefix(ByVal e As System.Data.DataColumnChangeEventArgs)
            If m_updating Then
                Return
            End If
            m_updating = True
            ...
            End If
            Dim msgServ As IMessageService = CType(ServiceManager.Services.GetService(GetType(IMessageService)), IMessageService)
            m_AccountBook.CodePrefix = e.ProposedValue.ToString
            m_updating = False
        End Sub
...
#Region "Event Handlers"
        Private Sub ibtnBlank_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles ibtnBlank.Click
            Dim newItem As New AccountBook
            Me.m_accountbookCollection.Add(newItem)
            Dim newRow As TreeRow = Me.m_treeManager.Treetable.Childs.Add
            newRow.Tag = newItem
            Me.m_AccountBook = newItem
            Me.m_treeManager.Treetable.AcceptChanges()
        ...
        Public Sub UpdateItemRow()
            ...
            If itr Is Nothing Then
                Return
            End If
            If Me.m_AccountBook Is Nothing Then
                Return
            End If
            Me.m_tableInitialized = False
            itr("Linenumber") = Me.m_treeManager.Treetable.Childs.Count
            itr("code") = m_AccountBook.Code
            itr("Name") = m_AccountBook.Name
            itr("TitleName") = m_AccountBook.TitleName
            itr("Prefix") = m_AccountBook.CodePrefix
        ...
        Private Sub ibtnDelRow_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles ibtnDelRow.Click
            ...
            If theRow Is Nothing Then
                Return
            End If
            If Not TypeOf theRow.Tag Is AccountBook Then
                Return
            End If
            If Me.m_AccountBook Is Nothing Then
                Return
            End If
            Me.m_accountbookCollection.Remove(m_AccountBook)
            m_AccountBook = Nothing
        ...
        Private Sub tgItem_CurrentCellChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles tgItem.CurrentCellChanged
            ...
            If m_oldRow Is theRow Then
                Return
            End If
            If TypeOf theRow.Tag Is AccountBook Then
                m_AccountBook = CType(theRow.Tag, AccountBook)
            Else
                m_AccountBook = Nothing
            End If
        ...
        Private Sub btnSearch_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSearch.Click
            ...
            If otherLength > 0 Then
                ...
                Next
            End If
            Me.m_accountbookCollection = New AccountBookCollection(newfilters)

            Me.m_tableInitialized = False
            Me.m_accountbookCollection.PopulateTable(Me.m_treeManager.Treetable)
            Me.m_tableInitialized = True
        End Sub
...
#Region "Overrides"
        ...
        Public Overloads Overrides Sub Save()
            ...
            If Not Me.AccountBookCollection Is Nothing Then
                Dim errorMessage As String = Me.AccountBookCollection.Save(currentUserId).Message
