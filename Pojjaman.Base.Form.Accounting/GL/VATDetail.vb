...
Namespace Longkong.Pojjaman.Gui.Panels
  Public Class VATDetail
    ...
    Implements IValidatable, IAuxTab, IAuxTabItem, IPrintDocumentsList
...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Protected Sub InitializeComponent()
            ...
            Dim resources As System.ComponentModel.ComponentResourceManager = New System.ComponentModel.ComponentResourceManager(GetType(VATDetail))
            ...
            'btnVatGroupDialog
            '
            Me.btnVatGroupDialog.FlatStyle = System.Windows.Forms.FlatStyle.System
            ...
            'btnVatGroup
            '
            Me.btnVatGroup.FlatStyle = System.Windows.Forms.FlatStyle.System
...
#Region "Style"
    Public Function CreateTableStyle() As DataGridTableStyle
      ...
      csPrintName.TextBox.Name = "vati_printname"
            ...
            Dim csTaxId As New TreeTextColumn
            csTaxId.MappingName = "vati_taxId"
            csTaxId.HeaderText = myStringParserService.Parse("${res:Longkong.Pojjaman.Gui.Panels.VatDetail.TaxIdHeaderText}")
            csTaxId.NullText = ""
            csTaxId.Width = 80
            csTaxId.TextBox.Name = "vati_taxId"

            Dim csBranchId As New TreeTextColumn
            csBranchId.MappingName = "vati_branchId"
            csBranchId.HeaderText = myStringParserService.Parse("${res:Longkong.Pojjaman.Gui.Panels.VatDetail.BranchIdHeaderText}")
            csBranchId.NullText = ""
            csBranchId.Width = 40
            csBranchId.TextBox.Name = "vati_branchId"
      ...
      dst.GridColumnStyles.Add(csSupplierButton)
            ...
            dst.GridColumnStyles.Add(csTaxId)
            dst.GridColumnStyles.Add(csBranchId)
            ...
            End If
                    ...
                    Case "vati_branchid"
                        ...
                        If TypeOf Me.Entity Is SimpleBusinessEntityBase Then
                            CType(Entity, SimpleBusinessEntityBase).OnGlChanged()
                        End If
                        doc.BranchId = CInt(e.ProposedValue)
    ...
    Private Sub SetSupplier(ByVal supplier As ISimpleEntity)
            ...
            If doc Is Nothing Then
                ...
                If Me.m_vat.ItemCollection.Count = 0 Then
                    doc.TaxBase = Me.m_vat.RefDoc.TaxBase
                    Me.m_vat.ItemCollection.Add(doc)
                Else
                    Dim mVat As Decimal = 0
                    For Each row As TreeRow In Me.m_treeManager.Treetable.Rows
                        ...
                        End If
                    Next
                    If Me.m_vat.RefDoc.TaxBase - mVat >= 0 Then
                        doc.TaxBase = Me.m_vat.RefDoc.TaxBase - mVat
                        Me.m_vat.ItemCollection.Add(doc)
            ...
            doc.PrintName = CType(supplier, Supplier).Name
            doc.PrintAddress = CType(supplier, Supplier).BillingAddress
            doc.TaxId = CType(supplier, Supplier).TaxId
            doc.BranchId = CType(supplier, Supplier).BranchId

            m_isInitialized = False
            Me.m_treeManager.SelectedRow("vati_printname") = doc.PrintName
            Me.m_treeManager.SelectedRow("vati_printaddress") = doc.PrintAddress
            Me.m_treeManager.SelectedRow("vati_taxId") = doc.TaxId
            Me.m_treeManager.SelectedRow("vati_branchId") = Configuration.BranchString(doc.BranchId)
            If newdoc Then
                Me.m_treeManager.SelectedRow("vati_taxbase") = Configuration.FormatToString(doc.TaxBase, DigitConfig.Price)
            End If
...
#Region "IPrintable"
    ...
    Public Overrides ReadOnly Property CanPrint() As Boolean
      ...
      End Get
    End Property
    Public ReadOnly Property VatItemWithCustomNoteForPreview() As List(Of VatItemWithCustomNote) Implements IPrintDocumentsList.VatItemWithCustomNoteForPreview
      Get
        Dim ls As List(Of VatItemWithCustomNote) = New List(Of VatItemWithCustomNote)
        For i As Integer = 0 To Me.m_vat.ItemCollection.Count - 1
          If Not m_treeManager.Treetable.Rows(i).IsNull("Selected") Then
            If CBool(m_treeManager.Treetable.Rows(i)("Selected")) Then
              Dim item As VatItem = Me.m_vat.ItemCollection(i)
