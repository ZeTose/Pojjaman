...
namespace Longkong.ColorPicker.Controls {
	...
	public class ColorPanel : UserControl {
		...
		public ColorPanel(Color color) :this()
		{ 			
		...
		{
			get { return ColorConverter.RgbToColor((RGB)rgbColorSpace.Structure); }
			set
			{
				this.ColorSelected(value,true);
			}
		}
		...
		private void picColorSlider_MouseDown(object sender, System.Windows.Forms.MouseEventArgs e) {						
			...
			m_currentColorSliderArrowYLocation = this.m_colorSliderInnerRegion.Top + e.Y;
			...
			this.UpdateColorPanels( false, false, true );
			CheckCursorYRegion( e.Y + m_colorSliderInnerRegion.Top );
