...
Namespace Longkong.Pojjaman.BusinessLogic
  Public Class RptGLfromPayReceivebyEntity
    Inherits Report
...
#Region "Methods"
    ...
    Private Sub CreateHeader()
      ...
      '"รหัสเจ้าหนี้"
      m_grid(0, 1).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAPGL.APCode}")
      '"ชื่อเจ้าหนี้"
      m_grid(0, 2).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAPGL.APName}")
      '"วงเงินเครดิต"
      m_grid(0, 3).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAPGL.CreditAmount}")
      '"สถานะเจ้าหนี้"
      m_grid(0, 4).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAPGL.APStatus}")
      '"เลขที่ผังบัญชี"
      m_grid(1, 1).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAPGL.AcctCode}")
      '"ชื่อผังบัญชี"
      m_grid(1, 2).Text = Me.StringParserService.Parse("${res:Longkong.Pojjaman.BusinessLogic.RptAPGL.AcctName}")
      '"วันที่เอกสาร"
    ...
    Private Sub PopulateData()
      ...
      For Each row As DataRow In dt.Rows
        ...
        If Not ((CStr(row("EntityCode")) = currentEntityCode) _
          AndAlso (CInt(row("EntityType")) = currentEntityType)) Then  ' เปิดเอกสารใหม่
          If opnEntityIndex > -1 Then 'ก็จะไม่ใช่รอบแรก
            'ยอดสรุป Entity code ก่อนหน้า แลเปิด อันใหม่ และ ผังบัญชีอันใหม่ด้วย
            m_grid.RowCount += 1
            currDocIndex = m_grid.RowCount

            remainAccount = sumAccountCredit - sumAccountDebit
            ...
            m_grid(currDocIndex, 4).CellValue = "จำนวนผังบัญชี(ผัง)"
            ...
            m_grid(currDocIndex, 4).CellValue = countAccount
            'm_grid(currDocIndex, 5).CellValue = Configuration.FormatToString(opnEntity, DigitConfig.Price)
          ...
          m_grid(currDocIndex, 4).CellValue = row("EntityStatus")
          '----------------------------------------
          'm_grid.RowCount += 1
          'currDocIndex = m_grid.RowCount
          ...
          m_grid(currDocIndex, 4).CellValue = "ยอดยกมา"
          m_grid(currDocIndex, 8).CellValue = Configuration.FormatToString(opnAccount, DigitConfig.Price)
        ...
        ElseIf Not (CStr(row("AcctCode")) = currentAccountCode) Then
          'สรุปผังบัญชีก่อนหน้า และเปิดอันใหม่
          m_grid.RowCount += 1
          currDocIndex = m_grid.RowCount
          remainAccount += sumAccountCredit - sumAccountDebit
        ...
        If countRow = totalRow Then
          ...
          If opnEntityIndex > -1 Then
            m_grid(opnEntityIndex, 8).CellValue = Configuration.FormatToString(opnEntity, DigitConfig.Price)
...
#End Region#Region "Properties"    Public Overrides ReadOnly Property ClassName() As String
      Get
        Return "RptGLfromPayReceivebyEntity"
      End Get
    End Property
    Public Overrides ReadOnly Property DetailPanelTitle() As String
      Get
        Return "${res:Longkong.Pojjaman.BusinessLogic.RptGLfromPayReceivebyEntity.DetailLabel}"
      End Get
    End Property
    ...
    Public Overrides ReadOnly Property ListPanelTitle() As String
      Get
        Return "${res:Longkong.Pojjaman.BusinessLogic.RptGLfromPayReceivebyEntity.ListLabel}"
      End Get
    End Property
