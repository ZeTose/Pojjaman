            ...
            If Me.DataSet.Tables.Count > 0 Then
                ...
                For Each row As DataRow In dt.Select("gli_acct = " & id.ToString)
                    ...
                    If Not row.IsNull("gl_id") Then
                        ...
                        Dim sum As Decimal = cash + bank + remain + Other
                        Dim diff As Decimal = Math.Abs(gli_balanceamt - sum)
                        If diff <= 1 Then
                            sum = gli_balanceamt
                        End If
                        theRow2("sum") = Configuration.FormatToString(sum, DigitConfig.Price, True)
