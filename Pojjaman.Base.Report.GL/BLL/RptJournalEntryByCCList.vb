...
#Region "Overrides"
    ...
    '          If ccId > 0 Then
    '            tr = dt.Childs.Add
    '            tr("acct_code") = indent & CType(newCChash(ccId), DataRowHelper).GetValue(Of String)("cc_code")
    '            tr("acct_name") = CType(newCChash(ccId), DataRowHelper).GetValue(Of String)("cc_name")
    '            tr("amount") = Configuration.FormatToString(drh.GetValue(Of Decimal)("amount"), DigitConfig.Price)
    '            tr("rowType") = "costcenter"
    '          End If
    '        Next
    ...
    '            If showDocument AndAlso Not childNode Is Nothing Then
    '              For Each docrow As DataRow In dt5.Select("gli_acct=" & acctId.ToString & " and gli_cc=" & ccId.ToString)
    '                Dim docdrh As New DataRowHelper(docrow)

    '                childNode.State = RowExpandState.Expanded
    '                docNode = childNode.Childs.Add
    '                docNode("acct_code") = indent & indent & docdrh.GetValue(Of Date)("gl_refdate").ToShortDateString
    '                docNode("acct_name") = docdrh.GetValue(Of String)("gl_refcode")
    '                docNode("amount") = Configuration.FormatToString(docdrh.GetValue(Of Decimal)("gli_amt"), DigitConfig.Price) 'indent & indent & docdrh.GetValue(Of String)("entity_description")
    '                docNode("rowType") = "document"
